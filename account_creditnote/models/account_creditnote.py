...
#                             string='Principle', store=True, readonly=True),
                ...
                'state':fields.selection([
                    ...
                    ('claimed', 'Claimed'),
                        ...
                        'p_line':fields.one2many('account.creditnote.line', 'line_id', 'Principal Lines',
                              copy=True),
                }
    _defaults = {
        'type':'cash',
        ...
        if ids:
                for resVal in self.browse(cr, uid, ids, context=context):
                    if not resVal.principle_id:
                        raise osv.except_osv(_('Configuration Error!'), _('Please select the principal.'))
                    if resVal.p_line:
                        check_amount =0
                        ...
                        for line_data in resVal.p_line:   
                            account_move = {
                                                ...
                                                'branch_id': resVal.customer_branch_id.id,
                            ...
                            cr.execute("""insert into account_move_line (partner_id,name,account_id,date_maturity,move_id,credit,debit,journal_id,date,company_id,period_id,branch_id) 
                                values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s),
                                      (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
                                      (resVal.customer_id.id, resVal.name , line_data.principle_id.property_receivable_clearing_account.id, resVal.approved_date, move_id, 0.0, line_data.amount, journal_id[0], resVal.approved_date, company_id, period_id,resVal.customer_branch_id.id,
                                       resVal.customer_id.id, resVal.name, line_data.principle_id.property_trade_payable_account.id, resVal.approved_date, move_id, line_data.amount, 0.0, journal_id[0], resVal.approved_date, company_id, period_id,resVal.customer_branch_id.id,))
                        ...
                        cr.execute("""insert into account_move_line (partner_id,name,account_id,date_maturity,move_id,credit,debit,journal_id,date,company_id,period_id,branch_id) 
                            values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s),
                                  (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
                                  (resVal.customer_id.id, resVal.name , resVal.principle_id.property_receivable_clearing_account.id, resVal.approved_date, move_id, 0.0, resVal.amount, journal_id[0], resVal.approved_date, company_id, period_id,resVal.customer_branch_id.id,
                                   resVal.customer_id.id, resVal.name, resVal.principle_id.property_trade_payable_account.id, resVal.approved_date, move_id, resVal.amount, 0.0, journal_id[0], resVal.approved_date, company_id, period_id,resVal.customer_branch_id.id,))
...
class account_creditnote_line(osv.osv):  
    _name = 'account.creditnote.line'
    _columns = {                
        'line_id':fields.many2one('account.creditnote', 'Line', ondelete='cascade', select=True),
        'principle_id':fields.many2one('product.maingroup', 'Principal'),
        'amount' : fields.float(string='Amount', digits=(16, 0), readonly=False),
    }    