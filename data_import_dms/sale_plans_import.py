            ...
            try:
                for aml in amls:
                        ...
                        if aml['branch']:
                            ...
                            cr.execute('select id from sale_branch where lower(branch_code) like %s', (branch.lower(),))
                        ...
                        if day_plan and sale_team_id and main_group_id:
                            cr.execute('select id from sale_plan_day where active=True and lower(name) like %s and sale_team= %s and main_group = %s', (day_plan.lower(), sale_team_id, main_group_id,))
                            result = cr.fetchall()
                            if not result:
        ...
        if amls or err_log:
            ...
            try:
                for aml in amls:
                    ...
                    if day_plan and sale_team_id and main_group_id:
                        cr.execute('select id from sale_plan_trip where active=True and lower(name) like %s and sale_team= %s and main_group = %s', (day_plan.lower(), sale_team_id, main_group_id,))
                        result = cr.fetchall()
                        if not result:
                            plan_id = sale_plan_day_obj.create(cr, uid, {'name': day_plan,
                                                                                        'sale_team':sale_team_id,
