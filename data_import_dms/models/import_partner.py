...
header_fields = [ 'sale channel', 'postal code', 'region', 'class', 'outlet id code', 'outlet name', 
                 ...
                 'website', 'title', 'job position', 'old code', 'email', 'fax' , 'credit limit', 'credit allow']
...
#                 if township:  # no need to update
#                     township = township.strip()
#                     cr.execute("""select id from res_township where lower(name) like %s""", (township.lower(),))
#                     data = cr.fetchall()
#                     if data:
#                         township_ids = data[0][0]
#                     township_res = {'name':township}
#                     if not township_ids:
#                         township_id = township_obj.create(cr, uid, township_res, context)
#                     else:
