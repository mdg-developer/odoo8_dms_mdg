...
from .inventory_report_by_category import inventory_report_category
...
#         if not data['form']['filter_product_categ_ids']:
#             category_ids = self.pool['product.category'].search(self.cr, self.uid, [])
#         else:
#             category_ids=data['form']['filter_product_categ_ids']
        for line_wh in data['form']['filter_product_categ_ids']:
            ...
            gve=_p.get_value_exist(line_wh,cid[0])
            toal_val=_p.total_vals(cid[0])
            cat_name=_p.get_categ(line_wh)
            c_specs = [    
                            ('product', 2, 0, 'text', cat_name or ''),
...
stock_inventory_report_xls('report.stock_inventory_report.inventory_report_by_category_xls',
                   'stock.inventory.reports',
                   parser=inventory_report_category)
