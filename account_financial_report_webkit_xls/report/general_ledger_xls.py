...
class general_ledger_xls(report_xls):
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        c_specs += [
            ...
            ('aaf', 1, 0, 'text', _p.analytic_accounts(data) and ', '.join(
...
#             ('br', 1, 0, 'text', _p.branch_ids(data) and ', '.join(
#                 [branch.branch_code for branch in _p.branch_ids(data)]) or _('All')),
             ('br', 2, 0, 'text', branch_name),           
            #('br', 2, 0, 'text', _p.branch[1] if _p.branch else '-'),
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(
            ws, row_pos, row_data, row_style=cell_style_center)
            ...
            if _p.display_account_raw(data) == 'all' or \
                ...
                for line in _p['ledger_lines'][account.id]:
                    ...
                    c_specs += [
                        ...
                        ('account_code', 1, 0, 'text', line.get('account_code')or ''),
                        #TODO anlaytic
