    ...
    def print_header_data(self, ws, _p, data, row_position, xlwt, _xs,
        ...
        c_specs += [
            ...
            ('coa', 2, 0, 'text', _p.chart_account.name,
             None, cell_style_center),
            ('br', 2, 0, 'text', branch_name,#'' if _p.branch_id else '-',
             None, cell_style_center),        
#             ('br', 2, 0, 'text', _p.branch_id[1] if _p.branch_id else '-',
#              None, cell_style_center),
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_position = self.xls_write_row(
    ...
    def print_account_header(self, ws, _p, data,_xs, xlwt, row_position):
        ...
        if _p.amount_currency(data):
                    c_specs += [('curr_bal', 1, 0, 'text', _('Curr. Bal.'), None, cell_style_center),
                                ('curr_code', 1, 0, 'text', _('Curr.'), None, cell_style_center),]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_position = self.xls_write_row(
            ws, row_position, row_data, row_style=cell_style)
        return row_position
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for current_account in objects:
            ...
            row_pos = self.print_account_header(ws, _p, data,_xs, xlwt, row_pos)
            ...
            for (partner_code_name, partner_id, partner_ref, partner_name) \
                ...
                for line in _p['ledger_lines'][current_account.id].get(partner_id, []):
                    curbal += line.get('amount_currency') or 0.0
                    curcode = line.get('currency_code')
                    
                if curbal > 0.0:
                        c_specs += [('curr_bal', 1, 0, 'number', curbal, None,
                                 regular_cell_style_decimal),
                                ('curr_code', 1, 0, 'text', curcode or '', None,
                                 regular_cell_style),]           
