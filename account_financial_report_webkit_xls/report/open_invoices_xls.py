...
class open_invoices_xls(report_xls):
    ...
    def print_header_titles(self, _p, data, row_position):
        c_specs = [
            ...
            ('br', 3 if self.nbr_columns == 13 else 2, 0, 'text',
             _('Branch'), None, self.style_bold_blue_center),
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_position = self.xls_write_row(
    ...
    def print_header_data(self, _p, data, row_position):
        ...
        if _p.account_id:
            ...
            for account in account_obj.browse(self.cr,self.uid,_p.account_id,context=None):
                account_name += str(account.code) + ','
            account_name = account_name[:-1]    
        else:
            account_name = 'All' 
            
        c_specs = [
            ('coa', 2, 0, 'text', account_name,
...
#               None, self.style_center),
        ...
        c_specs += [
            ...
            ('br', 2, 0, 'text', branch_name, None, self.style_center),        
            #('br', 2, 0, 'text', _p.branch_id[1] if _p.branch_id else u'', None, self.style_center),
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_position = self.xls_write_row(
