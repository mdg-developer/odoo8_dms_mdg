...
class trial_balance_xls(report_xls):
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        try:
            ...
            #if _p.branch_ids:
            if data.get('form', False) and data['form'].get('branch_ids', False):
                for branch in branch_obj.browse(self.cr,self.uid,data['form']['branch_ids'],context=None):
                    branch_name += str(branch.branch_code) + ','
                branch_name = branch_name[:-1]    
            else:
                branch_name = 'All'         
        ...
        for current_account in objects:
            ...
            if _p.amount_currency(data):
                #if  _p['amt_curr_accounts'][current_account.id] != 0.0:
                #    curname=_p['name_curr_accounts'][current_account.id]
                c_specs += [
                        ('curr_bal', 1, 0, 'number', _p['amt_curr_accounts'][current_account.id] , None, regular_cell_style_decimal),
                        #('curr_code', 1, 0, 'text', curname , None, regular_cell_style),
                    ]      
            row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
            row_pos = self.xls_write_row(
                ws, row_pos, row_data, row_style=cell_style)
