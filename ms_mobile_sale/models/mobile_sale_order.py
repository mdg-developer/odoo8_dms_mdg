                ...
                for so in sale_order:
                    ...
                    mso_result = {
                      ...
                      #  'delivery_remark':so['delivery_remark'],
                        ...
                        'cash_discount':so['cashDisAmount'],
                        'user_id':so['user_id'],
                        'name':so['name'],
...
#             str = str.replace("}{", "}|{")
      #     str = str.replace(":'}{", ":''}")
#            new_arr = dict(str)
#             for key,value in new_arr.iteritems():
...
#                 data = data.replace("[", "")  # null
            ...
            cursor.execute("select id from stock_location where location_id = %s or id =%s ", (location_id, location_id))
...
#                        procurement_obj.run_scheduler(cursor, user, use_new_cursor=cursor.dbname, company_id = comp, context=context)                                      
                    ...
                    if uom:
                        ...
                        product_obj = product_product_obj.browse(cursor, user, product_id, context=context)
...
#            print 'YOOOOOOOOOOOOO', sale_ids
    ...
    def get_products_categ_by_sale_team(self, cr, uid, section_id , last_date, context=None, **kwargs):
...
#                     pi.categ_id,
#                     ((pi.price_surcharge/floor(1/pi_uom.factor))*(floor(1/uom.factor))) price_surcharge,
#                     rel.product_uom_id
...
#                     and rel.product_template_id=pi.product_tmpl_id
#                     and rel.product_uom_id=uom.id
#                     and pi_uom.id=pi.product_uom_id
#                     and pi.price_version_id = %s''', (version_id,))
#         datas = cr.fetchall()
#         return datas

    def get_pricelist_item_datas(self, cr, uid, version_id, context=None, **kwargs):
       cr.execute('''select CAST(RPAD(pi.id::text, 6, '0')||LPAD(pi.product_tmpl_id::text, 6, '0')||LPAD(rel.product_uom_id::text, 4, '0') AS bigint)as id,pi.price_discount,pi.sequence,pi.product_tmpl_id,pi.name,pp.id base_pricelist_id,
                    pi.product_id,pi.base,pi.price_version_id,pi.min_quantity,
                    pi.categ_id,
                    round((((pt.list_price * (1+coalesce(pi.price_discount,0)))+coalesce(pi.price_surcharge,0))/floor(1/pi_uom.factor))*(floor(1/uom.factor))) price_surcharge,
                    rel.product_uom_id
                    from product_pricelist_item pi,
                    product_pricelist_version pv,
                    product_pricelist pp,
    ...
    def get_company_datas(self, cr, uid , context=None):    
       ...
       if branch_code =='DN':
           ...
           cr.execute('''select id,name from res_company where name like 'Beauty World %' ''')
           datas = cr.fetchall()        
       return datas
...
#        print 'list_val', list_val
        ...
        try:
            ...
            cr.execute('''select distinct sp.id,sp.date,sp.partner_id,sp.company_id,sp.id,sp.name,sp.branch_id ,sp.state,sl.name as location ,pg.name as procurement
                             from stock_picking sp , stock_move sm ,stock_location sl  ,procurement_group pg
                             where sp.state  not in ('cancel','done')           
                             and sm.picking_id =sp.id         
                             and pg.id= sp.group_id
                ...
                for deli in deliver_data:       
                    ...
                    if deli['miss'] == 't':
                        ...
                        if So_id:
                            ...
                            # For DO
                            print 'So_idSo_idSo_idSo_id',So_id
...
#                 rental_collection.append(r)  
    ...
    def create_new_customer(self, cursor, user, vals, context=None):
        try:
            ...
            partner_branch_obj = self.pool.get('res.branch')
            str = "{" + vals + "}"
            str = str.replace("'',", "',")  # null
            str = str.replace(":',", ":'',")  # due to order_id
            str = str.replace("}{", "}|{")
            ...
            if new_partner:
                for partner in new_partner:
                    ...
                    if partner['branch_id'] :
                        branch = partner_branch_obj.browse(cursor,user,int(partner['branch_id']),context=None)
                        if branch:
                            if branch.name == 'Dean':
                                dean_cust = True,
                            else: 
        ...
        try:
            if partner_list: 
                ...
                cr.execute('''select A.id,A.name,A.image,A.is_company, A.image_small,replace(A.street,',',';') street,replace(A.street2,',',';') street2,A.city,A.website,
                     ...
                     A.is_consignment,A.hamper,A.is_bank,A.is_cheque,A.res_total_amount,A.distributor_target_amount,A.is_distributor
                     from (
                     ...
                     RP.hamper,RP.is_bank,RP.is_cheque,RP.res_total_amount,RP.distributor_target_amount,RP.is_distributor
    ...
    def product_qty_in_stock(self, cr, uid, warehouse_id , context=None, **kwargs):
            cr.execute("""
                            select product_id,ceiling(qty_on_hand*(floor(1/p.factor))) as qty_on_hand,main_group,name_template,price,sequence from
              (    
                select product.id as product_id,sum(qty) as qty_on_hand,product_temp.main_group as main_group,product_temp.uom_id as uom,
                         ...
                         group by quant.product_id, main_group,name_template,product.id,price,product.sequence,product_temp.uom_id
            )A left join product_uom p on p.id=A.uom
            where qty_on_hand > 0  order by name_template
            """, (warehouse_id,))   
            datas = cr.fetchall()        
            return datas
    ...
    def res_partners_team(self, cr, uid, section_id, late_date, context=None, **kwargs):
        ...
        select A.id,A.name,A.image,A.is_company, A.image_small,replace(A.street,',',';') street,replace(A.street2,',',';') street2,A.city,A.website,
                     ...
                     RP.is_bank,RP.is_cheque,RP.res_total_amount,RP.distributor_target_amount,RP.is_distributor
