...
#                             account_id=partner.property_account_receivable.id
#                             invoiceObj.write(cr,uid,invoice_id,{'account_id':account_id}, context)
                            cr.execute('update account_invoice set payment_type=%s , direct_credit =%s, branch_id =%s,delivery_remark =%s ,date_invoice=%s where id =%s', ('credit', ms_ids.direct_credit, ms_ids.branch_id.id, ms_ids.delivery_remark,de_date, invoice_id,))
                            invoiceObj.button_reset_taxes(cr, uid, [invoice_id], context=context)
#                             invlist = []
#                             invlist.append(invoice_id)
#                             # call the api function
...
#                         ''', (branch_id, status,))
            ...
            cr.execute('''select id,sequence as seq,from_date ,to_date,active,name as p_name,
                        ...
                        ''', (branch_id, status, team_id,))
        datas = cr.fetchall()
        print 'datasdatasdatasdatas',datas
        return datas
...
#                 invoice_ids = cursor.fetchall()
                cursor.execute("select order_id from pending_delivery pd,sale_order so where pd.order_id =so.id and so.payment_type ='cash' and miss !=True and pd.delivery_date=%s and pd.delivery_team_id =%s", (de_date, team_id,))
                invoice_ids = cursor.fetchall()                       
                if invoice_ids:
            ...
            if stock:
                for sr in stock:
                    ...
                    for srl in stock_line:  # return_quantity=  float(srl['return_quantity']) - (float(srl['sale_quantity'])+float(srl['foc_quantity'] ))
                            cursor.execute('select default_code From product_product where id  = %s ', (srl['product_id'],))
                            data = cursor.fetchall()
                            if data:
...
#                             stock_request_line_obj.create(cursor, user, mso_line_res, context=context)
                    ...
                    for srl in stock_line:
                        if (sr['rfi_no'] == srl['rfi_no']):
                            cursor.execute('select a.uom_ratio,a.uom_id,b.sequence from product_template a, product_product b where a.id = b.product_tmpl_id and b.id = %s ', (srl['product_id'],))
                            data = cursor.fetchall()
                            if data:
                                packing_unit = data[0][0]
                                small_uom_id = data[0][1]
                            ...
                            if  big_uom_qty:
                                req_quantity = ori_req_quantity * bigger_qty
