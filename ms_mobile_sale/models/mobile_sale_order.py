   ...
   #     'journal_id'  : fields.many2one('account.journal', 'Journal' ,domain=[('type','in',('cash','bank'))]),   
    ...
    def get_saleorder_status(self, cr, uid, sale_team_id , context=None, **kwargs):
        cr.execute('''
                    ...
                    sp.date_done as delivery_date,date_invoice invoice_date,so.state sale_order_state,so.schedule_date
    ...
    def get_customers_by_branch(self, cr, uid, branch_id , context=None, **kwargs):
        cr.execute('''
    ...
    def get_customers_by_branch_with_sync_date(self, cr, uid, branch_id , sync_date, context=None, **kwargs):
        cr.execute('''
                    select A.id,A.name,A.image,A.is_company, A.image_small,replace(A.street,',',';') street,replace(A.street2,',',';') street2,A.city,A.website,
                    replace(A.phone,',',';') phone,A.township,replace(A.mobile,',',';') mobile,A.email,A.company_id,A.customer, 
                    A.customer_code,A.mobile_customer,A.shop_name ,
    ...
    def get_additional_promos_with_sync_date(self, cr, uid, sync_date , context=None):
        cr.execute('''select id,product_id,amount,rule_id,condition,quantity from sale_optional_promotion where write_date::date >= %s''',(sync_date,))
        datas = cr.fetchall()
        return datas
...
#                                     mso_inv_PromoLineObj.create(cr, uid, mso_inv_promo_line_result, context=context)  
    ...
    def get_promos_datas_by_main_group(self, cr, uid , branch_id, state, team_id, main_group_id, context=None, **kwargs):
        ...
        datas = cr.fetchall()        
        return datas
    def get_promos_datas_by_main_group_with_sync_date(self, cr, uid , branch_id, state, team_id, main_group_id,sync_date, context=None, **kwargs):
        if state == 'approve':
            status = 'approve'
            cr.execute('''select id,sequence as seq,from_date ,to_date,active,name as p_name,
                        logic ,expected_logic_result ,special, special1, special2, special3 ,description,
                        ...
                        ''', (branch_id, status, main_group_id, sync_date, team_id,))
        else:
            status = 'draft'            
            cr.execute('''select id,sequence as seq,from_date ,to_date,active,name as p_name,
                        logic ,expected_logic_result ,special, special1, special2, special3 ,description,
