...
class nov_journal_print(report_sxw.rml_parse):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if data['analytic_account_ids']:
            self.analytic_account_ids = data['analytic_account_ids']
        else:
            self.analytic_account_ids = []    
        if data['branch_ids ']:
            self.branch_ids =  data['branch_ids '] 
        else:
            self.branch_ids =  [] 
    ...
    def _lines(self, object):
        ...
        if len(self.analytic_account_ids) > 0 and len(self.branch_ids) > 0:
            if len(self.analytic_account_ids) == 1:
                where_extra += " AND l.analytic_account_id = %s " % self.analytic_account_ids[0]
            else:
                where_extra += " AND l.analytic_account_id in %s " % str(tuple(self.analytic_account_ids))
            if len(self.branch_ids) == 1:
                where_extra += "AND l.branch_id = %s" % self.branch_ids[0]
            else:        
                where_extra += "AND l.branch_id in %s" % str(tuple(self.branch_ids))
            
        elif len(self.analytic_account_ids) > 0:  
            if len(self.analytic_account_ids) == 1:
                where_extra += " AND l.analytic_account_id = %s " % self.analytic_account_ids[0]
            else:
                where_extra += " AND l.analytic_account_id in %s " % str(tuple(self.analytic_account_ids))
    ...
    def _tax_codes(self, object):
        ...
        self.cr.execute(
            ...
            "INNER JOIN account_move am ON l.move_id = am.id "
            "WHERE l.period_id in %s AND l.journal_id=%s "
            "AND l.tax_code_id IS NOT NULL AND am.state IN %s" + where_extra,
    ...
    def _totals(self, field, object, tax_code_id=None):
        ...
        if len(self.analytic_account_ids) > 0 and len(self.branch_ids) > 0:
            ...
            if len(self.branch_ids) == 1:
                ...
                select += "AND l.branch_id in %s" % str(tuple(self.branch_ids))
        elif len(self.analytic_account_ids) > 0:  
            if len(self.analytic_account_ids) == 1:
                select += " AND l.analytic_account_id = %s " % self.analytic_account_ids[0]
            else:
