...
class account_print_journal_xls(orm.TransientModel):
    ...
    _columns = {
        ...
        'analytic_account_ids': fields.many2many(
            ...
            'account_print_journal_xls_analytic_rel',
            'journal_xls_id',
            'analytic_id',
            string='Filter on analytic accounts'),        
        'branch_ids': fields.many2many(
            'res.branch',
            'account_print_journal_xls_branch_rel',
            'journal_xls_id',
            'branch_id',
            string='Filter on branches'),                
    }
    _defaults = {
        'group_entries': True,
    }
    ...
    def check_record_filter(self,cr,uid,analytics,branchs,journal_id,period_id,context=None):
        move_line_obj = self.pool.get('account.move.line')
        move_line_ids = []
        if len(analytics) > 0 and len(branchs) > 0:
            move_line_ids = move_line_obj.search(cr, uid,
                                          [('journal_id', '=', journal_id),('period_id', '=', period_id),
                                           ('analytic_account_id', 'in', analytics),
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        wiz_analytic_account_ids = [j.id for j in wiz_form.analytic_account_ids]
        ...
        if print_by == 'fiscalyear':
            journal_fy_ids = []
            self.check_record_filter(cr,uid,wiz_analytic_account_ids,wiz_branch_ids,wiz_journal_ids,wiz_period_ids,context=None)
            ...
            datas.update({
                ...
                'analytic_account_ids' : wiz_analytic_account_ids,
