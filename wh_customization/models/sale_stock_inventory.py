...
class sale_stock_inventory(osv.osv):
    _name = "sale.stock.inventory"
    ...
    _columns = {
        ...
        'line_ids': fields.one2many('sale.stock.inventory.line', 'sale_inventory_id', 'Inventories', readonly=False, states={'done': [('readonly', True)]}, help="Inventory Lines.", copy=True),
        'move_ids': fields.one2many('stock.move', 'sale_inventory_id', 'Created Moves', help="Inventory Moves.", states={'done': [('readonly', True)]}),
        ...
        'location_dest_id': fields.many2one('stock.location', 'Inventoried Destination Location', required=True, readonly=True, states={'draft': [('readonly', False)]}),
    ...
    _defaults = {
        ...
        'state': 'draft',
        'company_id': lambda self, cr, uid, c: self.pool.get('res.company')._company_default_get(cr, uid, 'sale.stock.inventory', context=c),
    ...
    def reset_real_qty(self, cr, uid, ids, context=None):
        ...
        self.pool.get('sale.stock.inventory.line').write(cr, uid, line_ids, {'product_qty': 0})
        return True
    ...
    def post_inventory(self, cr, uid, inv, context=None):
        ...
        ctx = context.copy()                                        
        ctx['active_model'] = 'sale.stock.inventory'
        move_obj.action_done(cr, uid, [x.id for x in inv.move_ids if x.state != 'done'], context=ctx)

    def action_check(self, cr, uid, ids, context=None):
        """ Checks the inventory and computes the stock move to do
        @return: True
        """
        inventory_line_obj = self.pool.get('sale.stock.inventory.line')
    ...
    def _get_inventory_lines(self, cr, uid, inventory, context=None):
        ...
        for product_line in cr.dictfetchall():
            ...
            product_line['sale_inventory_id'] = inventory.id
...
class sale_stock_inventory_line(osv.osv):
    _name = "sale.stock.inventory.line"
    _description = "Inventory Line"
    _order = "sale_inventory_id, location_name, product_code, product_name, prodlot_name"

    def _get_product_name_change(self, cr, uid, ids, context=None):
        return self.pool.get('sale.stock.inventory.line').search(cr, uid, [('product_id', 'in', ids)], context=context)

    def _get_location_change(self, cr, uid, ids, context=None):
        return self.pool.get('sale.stock.inventory.line').search(cr, uid, [('location_id', 'in', ids)], context=context)

    def _get_prodlot_change(self, cr, uid, ids, context=None):
        return self.pool.get('sale.stock.inventory.line').search(cr, uid, [('prod_lot_id', 'in', ids)], context=context)
    ...
    _columns = {
        'sale_inventory_id': fields.many2one('sale.stock.inventory', 'Inventory', ondelete='cascade', select=True),
        ...
        'company_id': fields.related('sale_inventory_id', 'company_id', type='many2one', relation='res.company', string='Company', store=True, select=True, readonly=True),
        'prod_lot_id': fields.many2one('stock.production.lot', 'Serial Number', domain="[('product_id','=',product_id)]"),
        'state': fields.related('sale_inventory_id', 'state', type='char', string='Status', readonly=True),
        'theoretical_qty': fields.function(_get_theoretical_qty, type='float', digits_compute=dp.get_precision('Product Unit of Measure'),
                                           store={'sale.stock.inventory.line': (lambda self, cr, uid, ids, c={}: ids, ['location_id', 'product_id', 'package_id', 'product_uom_id', 'company_id', 'prod_lot_id', 'partner_id'], 20),},
        ...
        'product_name': fields.related('product_id', 'name', type='char', string='Product Name', store={
                                                                                            ...
                                                                                            'sale.stock.inventory.line': (lambda self, cr, uid, ids, c={}: ids, ['product_id'], 20),}),
        ...
        'location_name': fields.related('location_id', 'complete_name', type='char', string='Location Name', store={
                                                                                            ...
                                                                                            'sale.stock.inventory.line': (lambda self, cr, uid, ids, c={}: ids, ['location_id'], 20),}),
        'prodlot_name': fields.related('prod_lot_id', 'name', type='char', string='Serial Number Name', store={
                                                                                            ...
                                                                                            'sale.stock.inventory.line': (lambda self, cr, uid, ids, c={}: ids, ['prod_lot_id'], 20),}),
    }
    ...
    def create(self, cr, uid, values, context=None):
        product_obj = self.pool.get('product.product')
        dom = [('product_id', '=', values.get('product_id')), ('sale_inventory_id.state', '=', 'confirm'),
        ...
        return super(sale_stock_inventory_line, self).create(cr, uid, values, context=context)
    ...
    def _resolve_inventory_line(self, cr, uid, inventory_line, context=None):
        ...
        vals = {
            'name': _('INV:') + (inventory_line.sale_inventory_id.name or ''),
            'product_id': inventory_line.product_id.id,
            'product_uom': inventory_line.product_uom_id.id,
            'date': inventory_line.sale_inventory_id.date,
            'company_id': inventory_line.sale_inventory_id.company_id.id,
            'sale_inventory_id': inventory_line.sale_inventory_id.id,
            'state': 'confirmed',
            'restrict_lot_id': inventory_line.prod_lot_id.id,
            'restrict_partner_id': inventory_line.partner_id.id,
         }
        inventory_location_id = inventory_line.sale_inventory_id.location_id.id#inventory_line.location_id.id#inventory_line.product_id.property_stock_inventory.id
        if diff < 0:
            #found more than expected
            vals['location_id'] = inventory_location_id
            vals['location_dest_id'] = inventory_line.sale_inventory_id.location_dest_id.id#inventory_line.location_id.id
            vals['product_uom_qty'] = -diff
        else:
            #found less than expected
            vals['location_id'] = inventory_line.sale_inventory_id.location_id.id
            vals['location_dest_id'] = inventory_line.sale_inventory_id.location_dest_id.id#inventory_location_id
            vals['product_uom_qty'] = diff
        ...
        if diff > 0:
            ...
            preferred_domain_list = [[('reservation_id', '=', False)], [('reservation_id.sale_inventory_id', '!=', inventory_line.sale_inventory_id.id)]]
