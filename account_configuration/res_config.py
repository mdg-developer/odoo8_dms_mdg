...
class res_company(osv.osv):
    ...
    _columns = {
        ...
        'discount_cash_account_id': fields.many2one(
            ...
            domain="[('type', '=', 'other')]",),
        'p_foc_account_id': fields.many2one(
            'account.account',
            string="FOC Account",
            domain="[('type', '=', 'other')]",),         
    }
...
class account_config_settings(osv.osv_memory):
    ...
    def onchange_discount_foc(self, cr, uid, ids, module_sale_foc_discount, context=None):
        ...
        return res
    def onchange_purchase_discount_foc(self, cr, uid, ids, module_purchase_account_foc, context=None):
        res = {}
        if not module_purchase_account_foc:
            res['value'] = {'p_foc_account_id': False}
        return res    
    _columns = {
      ...
      'discount_account_id': fields.related(
        ...
        'discount_cash_account_id': fields.related(
            ...
            string="Deduction Account", 
            domain="[('type', '=', 'other')]"),
     'module_purchase_account_foc': fields.boolean("Allow Purchase FOC",help="Allows you purchase foc environment."),
        'p_foc_account_id': fields.related(
            'company_id', 'p_foc_account_id',
            type='many2one',
            relation='account.account',
            string="FOC Account", 
            domain="[('type', '=', 'other')]"),            
    ...
    def onchange_company_id(self, cr, uid, ids, company_id, context=None):
        ...
        if company_id:
            ...
            res['value'].update({'foc_account_id': company.foc_account_id and company.foc_account_id.id or False, 
                                 ...
                                 'p_foc_account_id': company.p_foc_account_id and company.p_foc_account_id.id or False,})
        else: 
            res['value'].update({'foc_account_id': False, 
                                 'discount_account_id': False,
                                 'discount_cash_account_id': False,
                                 'p_foc_account_id':False})
        return res
