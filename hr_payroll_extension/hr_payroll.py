...
class hr_payslip(osv.osv):
    ...
    def _get_late_policy(self, policy_group, dDay):
        "Return an Late policy with an effective date before dDay but greater than all others"

        return self._get_policy(policy_group, policy_group.late_policy_ids, dDay)
    ...
    def _book_holiday_hours(
        self, cr, uid, contract, presence_policy, ot_policy, late_policy, attendances,
                            holiday_obj, dtDay, rest_days, lsd,
                            worked_hours, context=None):
    ...
    def leaves_list_init(self, cr, uid, employee_id, dFrom, dTo, tz, context=None):
        ...
        for leave in leave_obj.browse(cr, uid, leave_ids, context=context):
            res.append({
                ...
                'day_temp':leave.number_of_days_temp,
            })
        #print 'leave_res',res
        return res    
    ...
    def leaves_list_get_hours(
        ...
        for l in leaves_list:
            ...
            #print 'dtLvBegin',dtLvBegin,dtLvEnd
           
            utcdt_dayS = timezone(l['tz']).localize(dtS).astimezone(utc)
            utcdt_dayE = timezone(l['tz']).localize(dtE).astimezone(utc)
            #print 'utcdt_dayS,utcEnd,utcdt_dayE',utcdt_dayS,utcEnd,utcdt_dayE,utcBegin
            if utcdt_dayS <= utcEnd and utcdt_dayE >= utcBegin:
                ...
                if utcBegin.date() < utcdt_dayS.date() and utcEnd.date() > utcdt_dayS.date():
                    ...
                    hours = float((utcEnd - utcBegin).seconds / 60) / 60.0
                    #print 'hour5',hours
    ...
    def get_worked_day_lines(self, cr, uid, contract_ids, date_from, date_to, context=None):
        ...
        if next_year[0] is not None: 
            ...
            cr.execute("SELECT (date_part('year',age('%s-03-01', (trial_date_end +interval '1 days')::date)) * 12 )+date_part('month',age('%s-03-01', (trial_date_end +interval '1 days')::date)) as w_month from hr_contract where employee_id=%s",(next_year,next_year,employee_id,))
            w_month=cr.fetchone()
            if w_month:
                if w_month[0]>=12:
                    working_month=12;
            ...
            for day in range(0, nb_of_days):
                ...
                for pcode, pname, ptype, prate, pduration in presence_codes:
                    if attendances.get(pcode, False):
                        continue
                    if ptype == 'normal':
                        cr.execute ('''select count(the_day) from (select generate_series(%s::date, %s ::date, '1 day') as the_day) days
                        where extract('dow' from the_day) in (6)''',(day_from,day_to))  
                ...
                if working_hours_on_day:
                        ...
                        for line in ot_policy.line_ids:
                            ...
                            if line.type == 'daily' and ot_day:
                                ...
                                if partial_hr > 0:
                                    ...
                                    #print 'Number of Hours Shown on payslip For Shift is =====' ,attendances[line.code]['number_of_hours']
                    ...
                    if not done and daily_late and late_policy:
                        ...
                        for line in late_policy.line_ids:
                            ...
                            if line.type == 'daily' and pay_day and working_hours_on_day > active_after_hrs and line.active_start_time:
                        ...
                        for line in late_policy.line_ids:
                            active_after_hrs = float(line.active_after) / 60.0
                            if line.type == 'daily' and hours_after_late > active_after_hrs and not line.active_start_time:
                                attendances[line.code][
                    ...
                    lsd.push(False)
                leave_type, leave_hours,day_temp = self.leaves_list_get_hours(
                    cr, uid, contract.employee_id.id,
                ...
                #print 'as_day',as_day     
                #print 'leave_type',leave_type,scheduled_hours,working_hours_on_day ,rest_days
...
#                         leaves[leave_type]['total_month_day'] =as_day
#                            
#                             
#                     else:
#                         leaves[leave_type] = {
...
#                             'total_month_day':as_day,
#                             'contract_id': contract.id,
#                         }

                    if leave_type in leaves and day_temp>=1:
                        print 'this_condition'
                        leaves[leave_type]['number_of_days'] += 1
                        leaves[leave_type]['number_of_hours'] += (
                         leave_hours > scheduled_hours) and scheduled_hours or leave_hours
                    elif leave_type in leaves and day_temp<1:
                        print 'this_condition1'
                ...
                if leave_type and dtDateTime.weekday() in rest_days:    
                    
                    #print 'day_temp1222',day_temp
                    #print 'leaves',leaves                                     
                    if leave_type in leaves and day_temp>=1:
                        #print 'this_condition11111111111111',
                        leaves[leave_type]['number_of_days'] += 0
                        leaves[leave_type]['number_of_hours'] += 0
                        leaves[leave_type]['total_month_day'] +=1                         
                    elif leave_type in leaves and day_temp<1:
                        #print 'this_condition222222222222222222'
                        
                        leaves[leave_type]['number_of_days'] += 0
                        leaves[leave_type]['number_of_hours'] += 0
                        leaves[leave_type]['total_month_day'] +=day_temp                         
                         
                    elif day_temp>=1: 
                        #print 'this_condition333333333333333333333'
                        ...
                        #print 'this_condition4444444444444'
                ...
                if dtDateTime.weekday() not in rest_days and not public_holiday:
                    attendances['MAX'][
                        'number_of_hours'] += normal_working_hours
                    attendances['MAX']['number_of_days'] += 1
                    
                #attendances['MAX']['number_of_days'] = attendances['MAX']['number_of_days']-public_count
                    

            leaves = [value for key, value in leaves.items()]
            attendances = [value for key, value in attendances.items()]
            rest = [value for key, value in rest.items()]
            
            res += attendances + leaves +rest
        return res
...
#         print 'Checking attendance data only date equal or not is -----',new_sin , new_sout
        ...
        for i in range(0, len(sin)):
            ...
            if(int(start.weekday()) == int(ot_day)):
              ...
              if(working_hours_on_day>=4.0 and working_hours_on_day<8.0+1):
