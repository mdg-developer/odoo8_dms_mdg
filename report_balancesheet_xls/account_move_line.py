...
class account_move_line(osv.osv):
    ...
    def _query_get(self, cr, uid, obj='l', context=None):
        ...
        if context.get('analytic_account_ids',False):
            analytic_account_ids = context['analytic_account_ids']    
            query += ' AND '+obj+'.analytic_account_id IN (%s)' % ','.join(map(str, context['analytic_account_ids']))
        
        if context.get('account_analytic_id',False):
            analytic_account_ids = context['account_analytic_id']    
            query += ' AND '+obj+'.analytic_account_id IN (%s)' % ','.join(map(str, context['account_analytic_id']))    
