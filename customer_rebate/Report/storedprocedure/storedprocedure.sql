...
CREATE OR REPLACE FUNCTION customer_rebate_report(IN param_from_date date, IN param_to_date date)
  RETURNS TABLE(id bigint, customer_id integer, customer_name character varying, product_name character varying, qty integer, rebate_amount numeric, promotion character varying) AS
$BODY$
declare 
	so_data record ;
	rebate_data record;
	...
	begin  
		...
		FOR so_data IN
			select (date_order+'6 hour'::interval+'30 minutes'::interval)::date as sale_order_date,so.name as so_name,res.id as customer_id,res.name as customer_name,pp.id as product_id,pp.name_template as product_name,product_uos_qty,uom.name as uom
		...
		LOOP				
			FOR rebate_data IN	 
				select config.code as promotion,sale_qty,uom.name,comparator,exps.rebate_amount
				...
				and product_id=so_data.product_id
				...
				IF LOWER(so_data.uom)=LOWER('Viss') AND LOWER(rebate_data.name)=LOWER('Viss') THEN						
					IF rebate_data.comparator= '==' THEN						
						IF so_data.product_uos_qty = rebate_data.sale_qty THEN
							...
							insert into rebate_temp(promotion,customer_id,customer_name,product_name,qty,rebate_amount) values (rebate_data.promotion,so_data.customer_id,so_data.customer_name,so_data.product_name,so_data.product_uos_qty,amount);	
						END IF;	
