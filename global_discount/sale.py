...
class sale_order(orm.Model):
    ...
    def _calc_amount_untaxed_discounted(self, cr, uid, ids,
        ...
        for sale in self.browse(cr, uid, ids, context=context):
            ...
            discount = ((100 - sale.global_discount_percentage) / 100) + sale.global_discount_amount
    ...
    def _calc_amount_total_discounted(self, cr, uid, ids,               #These function only display the discounted amount not contain calculation
                                      name, args, context=None):        #
        if context is None:                                             #
            context = {}                                                #
        res = {}
    ...
    def _check_if_discount(self, cr, uid, ids,                          #customize the discount percentage and discount amount
                           name, args, context=None):                   #    
        if context is None:                                             #
            context = {}                                                #
        res = {}
            ...
            if sale.global_discount_percentage:
                ...
                for line in sale.order_line:
                    if line.global_discount == True:
                        res[sale.id] = True
                        break
            if sale.global_discount_amount:                 #This is extra adding for discount amount.
                res[sale.id] = True                         #        
            else:                                           #
                for line in sale.order_line:
                    if line.global_discount == True:
    ...
    _columns = {
                'global_discount_amount':fields.float('Discount Amount',readonly=True,states={
    ...
    def _create_global_discount_amount_lines_by_taxes(self, cr, uid, discount,
                                               line_by_taxes, product_id,
                                               context=None):
        if context is None:
            context = {}
        ...
        for tax_str in line_by_taxes.keys():
            ...
            line = False
            for line in line_by_taxes[tax_str]:
                qty = line.product_uom_qty#this is number of product
                pu = line.price_unit  #this is unit price per product
                sub = qty * pu # this is total amount of normal calculation not contain discount
                line_sum += sub
                discount_value =discount * qty
            if line:
                order = line.order_id
                res_value = line_obj.product_id_change(cr, uid, [],
                    pricelist=order.pricelist_id.id,
    ...
    def generate_global_discount(self, cr, uid, ids, context=None):
        ...
        product_ids = product_obj.search(cr,uid,[('default_code','=','global_discount'),])
        if len(product_ids) >0: 
            discount_product = product_ids[0]
            for sale in self.browse(cr, uid, ids, context=context):
                ...
                if sale.state in ('draft','sent') and \
                    sale.global_discount_amount != 0.00:
                    discount = sale.global_discount_amount#this amount is not percentage so we need to get only full amount
                    global_line_ids = line_obj.\
                        search(cr, uid, [
                            ('order_id', '=', sale.id),
                            ('product_id', '=', discount_product),
                    ...
                    line_by_taxes = self.\
                        _get_lines_by_taxes(cr, uid, sale.id, context=context)
                    discount_line_ids = self.\
                        _create_global_discount_amount_lines_by_taxes(cr, uid, discount,
                                                               line_by_taxes,
                                                               discount_product,
                                                               context=context)
                    self.write(cr, uid, sale.id, {
...
#     def generate_global_discount_amount(self, cr, uid, ids, context=None):
#         if context is None:
#             context = {}
#         line_obj = self.pool.get('sale.order.line')
#         data1_obj = self.pool.get('ir.model.data')
#         
#         for sale in self.browse(cr, uid, ids, context=context):
#             if sale.state in ('draft','sent') and \
#                 sale.global_discount_amount != 0.00:
#                 model, product_id = data1_obj.\
#                     get_object_reference(cr, uid,
#                                          'global_amount',
#                                          'product_global_amount')
...
#         return True
    ...
    def _make_invoice_amount(self, cr, uid, order, lines, context=None):
        if context is None:
            context = {}
        inv_id = super(sale_order, self).\
            _make_invoice_amount(cr, uid, order, lines, context=None)
        if order.global_discount_percentage:
            invoice_obj = self.pool.get('account.invoice')
            data_obj = self.pool.get('ir.model.data')
            model, product_id = data_obj.\
                get_object_reference(cr, uid,
                                     'global_discount',
                                     'product_global_discount')
            discount = order.global_discount_amount
            line_by_taxes = invoice_obj.\
                    _get_lines_by_taxes(cr, uid, inv_id,
                                        context=context)
            invoice_ids = invoice_obj.\
                _create_global_discount_amount_lines_by_taxes(cr, uid,
                                                       discount,
                                                       line_by_taxes,
                                                       product_id,
                                                       context=context)
...
class sale_order_line(orm.Model):
    ...
    _columns = {
        'global_discount': fields.boolean('Special Discount', readonly=True),
    }
