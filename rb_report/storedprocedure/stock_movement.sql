...
CREATE OR REPLACE FUNCTION stock_movement()
  RETURNS TABLE(x_id integer, x_date timestamp without time zone, x_location_id integer, x_product_id integer, x_source character varying, x_uom integer, x_from character varying, x_move_in_type character varying, x_transfer_in numeric, x_to character varying, x_move_out_type character varying, x_transfer_out numeric, x_balance numeric) AS
$BODY$
BEGIN
    RETURN QUERY
select s.id as x_id,
   ...
   (
       ...
       from
       (
           select s.id,s.location_id,s.product_id,s.date, s.origin ,s.product_uom
           from stock_move s,
           stock_location fl
           where s.state='done'
           ...
           and fl.usage!='customer'
            and fl.usage='internal'
           union
           select s.id,s.location_dest_id,s.product_id,s.date, s.origin,s.product_uom
           from stock_move s,
           stock_location tl
           where s.state='done'
   ...
   left join
   (
   select s.id,s.product_id,s.location_dest_id, s.product_qty as qty, s.product_uom,
...
--    and pt.uom_id=duom.id
   ...
   left join
   (
   select s.id,s.product_id,s.location_id, s.product_qty as qty,s.product_uom,
--    case when duom.id=suom.id then s.product_qty
--    when duom.id!=suom.id then (s.product_qty/suom.factor)*duom.factor end as qty, 
