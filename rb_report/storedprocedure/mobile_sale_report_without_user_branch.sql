...
Begin
  delete from report_temp1;
	...
	(select msol.product_id,
		...
		left join (select id ,name as branch_code from res_branch)D on C.branch_id=D.id)E 
	...
	left join (select res.id,res.name customer_name,branch.name as branch_name from res_partner res,res_branch branch where res.branch_id=branch.id )B on A.customer_id=B.id)C 
	left join (select id,name as tablet_name from tablets_information)D on D.id=C.tablet_id)E 
	left join (select id,name as sale_team_name from crm_case_section)F on F.id=E.sale_team)G
	...
	ELSEIF m_group IS NULL AND param_branch IS NOT NULL THEN
		return  query select * from report_temp1 report where report.branch_code in (select name from res_branch where id=param_branch) order by sale_date,order_id;
	ELSEIF m_group IS NOT NULL AND param_branch IS NOT NULL THEN
		return  query select report.* from report_temp1 report,product_product pp,product_template pt where pp.id=report.product_id and pt.id=pp.product_tmpl_id and pt.main_group=m_group and report.branch_code=(select name from res_branch where id=param_branch) order by sale_date,order_id;
	END IF;
