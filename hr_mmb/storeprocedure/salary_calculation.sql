...
--select * from salary_calculation('2016-02-01','2016-02-29')
...
CREATE OR REPLACE FUNCTION salary_calculation(
  ...
  RETURNS TABLE(id bigint, emp_cla integer, job character varying, p_job_id integer, total_emp integer, basic_salary numeric, allowance numeric,travell_amount numeric ,contribution numeric, cla numeric, total numeric, ssb numeric, total_value numeric, income_tax numeric) AS
$BODY$
DECLARE 
	emp_data_set RECORD;
	data_set RECORD;
	...
	--select* from employee_temp_table
	...
	loop
		insert into employee_temp_table(emp_cla,job_id,hp_id,job,emp_id,emp_name ,basic_salary ,allowance,travell_amount,contribution ,cla,total,ssb ,total_value,income_tax ,causlv,annuallv,unplv1,unplv2,medlv1,medlv2,medlv3,matlv,abslv1,abslv2,workld1,workld2,workld3,workld4,workld5)
	...
	--updating travelling amount
	for data_set in
	select ett.hp_id,hpl.code,hpl.total from hr_payslip_line hpl,employee_temp_table ett where hpl.slip_id=ett.hp_id and code='TAL'
	loop
	update employee_temp_table ett
	...
	--updating the sff
	for data_set in
	select hpi.amount,ett.hp_id from hr_payslip_input hpi,employee_temp_table ett where hpi.payslip_id=ett.hp_id and hpi.code='SFF'
	loop
	update employee_temp_table ett
...
-----Changes------
	...
	for data_set in 
	select ett.hp_id,hpw.code,hpw.number_of_days from hr_payslip_worked_days hpw,employee_temp_table ett where hpw.payslip_id=ett.hp_id and LOWER(code)='earnlv'
 	loop
 	update employee_temp_table ett
...
--	end if;
	...
	for data_set in 
	select ett.emp_cla,ett.job_id,ett.job,count(ett.job_id) as total_emp,sum(ett.basic_salary) as basic_salary,sum(ett.allowance) as allowance,sum(ett.travell_amount) as travell_amount,
	sum(ett.contribution) as contribution,sum(ett.cla) as cla,sum(ett.total) as total,sum(ett.ssb) as ssb,sum(ett.total_value) as total_value,
	sum(ett.income_tax) as income_tax from employee_temp_table ett group by ett.emp_cla,ett.job_id,ett.job,ett.travell_amount
	loop
	insert into employee_temp(emp_cla,job,job_id ,total_emp,basic_salary,allowance,travell_amount,contribution,cla,total,ssb,total_value,income_tax)
	values (data_set.emp_cla,data_set.job,data_set.job_id ,data_set.total_emp,data_set.basic_salary,data_set.allowance,
	data_set.travell_amount,data_set.contribution,data_set.cla,data_set.total,data_set.ssb,data_set.total_value,data_set.income_tax);
	end loop;

	--updating the total and total_value to employee_temp
	for data_set in 
	select et.job_id,(et.basic_salary+et.allowance+et.contribution+et.cla+et.travell_amount) as total,((et.basic_salary+et.allowance+et.contribution+et.cla+et.travell_amount)-(et.income_tax+et.ssb)) as total_value from employee_temp et
	loop
	...
	RETURN QUERY select row_number() over (order by job_id) as id,* from employee_temp order by job_id;
