...
class report_event_registration(models.Model):
    ...
    event_date = fields.Datetime('Event Start Date', readonly=True)
    event_end_date = fields.Datetime('Event End Date', readonly=True)
    ...
    exam_remark = fields.Integer('Exam Remark')
    remark = fields.Char('Remark', readonly=True)
    location=fields.Many2one('res.partner', 'Location', readonly=True)
    fee = fields.Float('Honorable Fee')
    other_fee=fields.Float('Other Fee')
    certificate = fields.Char('Certificate', readonly=True)
    tr_location = fields.Char('Training & Seminar', readonly=True)
    ...
    def init(self, cr):
        ...
        cr.execute(""" CREATE VIEW report_event_registration AS (
             select *,fee+other_fee as total_fee from(

 SELECT
                r.employee_id as employee_id,
                r.job_id as job_id,
                r.exam_remark as exam_remark,
                r.remark as remark,
                e.address_id as location,
                e.fee as fee,
                e.other_fee as other_fee,
                e.certificate as certificate,
                e.status as tr_location,
