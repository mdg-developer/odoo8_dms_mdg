...
#             tmpl_images and data.update({"images":tmpl_images})                                                                   
            ...
            if template.woo_brand_id:
                woo_brand = template.woo_brand_id
                if woo_brand:
                    if not woo_brand.woo_brand_id:
                        woo_brand_name = woo_brand.name
                        woo_description = woo_brand.description
                        url = instance.host+"/wp-json/product/v1/brands"
                        headers = CaseInsensitiveDict()
                        headers["Content-Type"] = "application/json"
                        row_data = {'name': woo_brand_name, 'description': str(woo_description) or ''}
                        res = requests.post(url, json=row_data, headers=headers)
                        if res.status_code not in [200,201]:
                            if res.status_code == 500:
                                response = res.json()
                                if isinstance(response,dict) and response.get('code')=='term_exists':
                                    woo_brand.write({'woo_brand_id':response.get('data'),'exported_in_woo':True})
                                    continue
                                else:
                                    message = res.content
                                    transaction_log_obj.create(
                        ...
                        response = res.json()
                        if response:
                            woo_brand_id =response.get('id',False)
                            slug = response.get('slug',False)
                            if woo_brand_id:
                                woo_brand.write({'woo_brand_id': woo_brand_id, 'exported_in_woo': True, 'slug':slug})
                                data.update({'brand_id':woo_brand_id,'brand_name':woo_brand_name})
                    else:
                        woo_brand_id = woo_brand.woo_brand_id
                        woo_brand_name = woo_brand.name
                        data.update({'brand_id':woo_brand_id,'brand_name':woo_brand_name})

            if template.woo_supplier_id:
                woo_supplier = template.woo_supplier_id
                if woo_supplier:
                    if not woo_supplier.woo_supplier_id:
                        woo_supplier_name = woo_supplier.name
                        woo_description = woo_supplier.description
                        url = instance.host+"/wp-json/product/v1/supplier"
                        headers = CaseInsensitiveDict()
                        headers["Content-Type"] = "application/json"
                        row_data = {'name': woo_supplier_name, 'description': str(woo_description) or ''}
                        res = requests.post(url, json=row_data, headers=headers)
                        if res.status_code not in [200,201]:
                            if res.status_code == 500:
                                response = res.json()
                                if isinstance(response,dict) and response.get('code')=='term_exists':
                                    woo_supplier.write({'woo_supplier_id':response.get('data'),'exported_in_woo':True})
                                    continue
                                else:
                                    message = res.content
                                    transaction_log_obj.create(
                        ...
                        if response:
                            ...
                            slug = response.get('slug',False)
                            if woo_supplier_id:
                                woo_supplier.write({'woo_supplier_id': woo_supplier_id, 'exported_in_woo': True, 'slug':slug})
                                data.update({'supplier_id':woo_supplier_id,'supplier_name':woo_supplier_name})
                    else:
                        woo_supplier_id = woo_supplier.woo_supplier_id
                        woo_supplier_name = woo_supplier.name
                        data.update({'supplier_id':woo_supplier_id,'supplier_name':woo_supplier_name})

            if template.woo_department_id:
                woo_department = template.woo_department_id
                if woo_department:
                    if not woo_department.woo_department_id:
                        woo_department_name = woo_department.name
                        woo_description = woo_department.description
                        url = instance.host+"/wp-json/product/v1/department"
                        headers = CaseInsensitiveDict()
                        headers["Content-Type"] = "application/json"
                        row_data = {'name': woo_department_name, 'description': str(woo_description) or ''}
                        res = requests.post(url, json=row_data, headers=headers)
                        if res.status_code not in [200,201]:
                            if res.status_code == 500:
                                response = res.json()
                                if isinstance(response,dict) and response.get('code')=='term_exists':
                                    woo_department.write({'woo_department_id':response.get('data'),'exported_in_woo':True})
                                    continue
                                else:
                                    message = res.content
                                    transaction_log_obj.create(
                        ...
                        if response:
                            ...
                            if woo_department_id:
                                woo_department.write({'woo_department_id': woo_department_id, 'exported_in_woo': True, 'slug':slug})
                                data.update({'department_id':woo_department_id,'department_name':woo_department_name})
                    else:
                        woo_department_id = woo_department.woo_department_id
                        woo_department_name = woo_department.name
                        data.update({'department_id':woo_department_id,'department_name':woo_department_name})
...
#                         info.update({"images":[{'id':var_url,'position': 0}]})
    ...
    def export_products_in_woo(self,instance,woo_templates,update_price,update_stock,publish,update_image=False):
        ...
        for woo_template in woo_templates:
            ...
            if woo_template.woo_brand_id:
                woo_brand = woo_template.woo_brand_id
                if woo_brand:
                    if not woo_brand.woo_brand_id:
                        woo_brand_name = woo_brand.name
                        woo_description = woo_brand.description
            ...
            if woo_template.woo_supplier_id:
                woo_supplier = woo_template.woo_supplier_id
                if woo_supplier:
                    if not woo_supplier.woo_supplier_id:
                        woo_supplier_name = woo_supplier.name
                        woo_description = woo_supplier.description
            ...
            if woo_template.woo_department_id:
                woo_department = woo_template.woo_department_id
                if woo_department:
                    if not woo_department.woo_department_id:
                        woo_department_name = woo_department.name
                        woo_description = woo_department.description
...
#                     data.update({'regular_price':variant.product_id.product_tmpl_id.ecommerce_price,'sale_price':variant.product_id.product_tmpl_id.ecommerce_price})
            ...
            _logger.info('<<<<<<<<<<<<<<<<<Call to wordpress export_products_in_woo here>>>>>>>>>>>>>>>>')
            _logger.info('message------> %s,',new_product.content)
    ...
    def export_new_products_in_woo(self,instance,woo_templates,update_price,update_stock,publish,update_image=True):
        ...
        for woo_template in woo_templates:
            ...
            _logger.info('<<<<<<<<<<<<<<<<<Call to wordpress export_new_products_in_woo here>>>>>>>>>>>>>>>>')
            _logger.info('message------> %s,',new_product.content)
            if not isinstance(new_product,requests.models.Response):               
                transaction_log_obj.create({'message': "Export Product\nResponse is not in proper format :: %s"%(new_product),
                                             'mismatch_details':True,
