...
class hr_employee(osv.Model):
    ...
    _columns = {
        ...
        'job_history_ids': fields.one2many('hr.employee.job.history', 'employee_id', 'Job History Records'),
        ...
        'emp_spouse_tax_exemption':fields.boolean('Tax Exemption') ,   
        'emp_father_tax_exemption':fields.boolean('Tax Exemption') ,   
        'emp_mother_tax_exemption':fields.boolean('Tax Exemption') ,    
    ...
    def state_inactive(self, cr, uid, ids, context=None):
        ...
        for d in ids:
            data = self.read(
                cr, uid, d, ['status', 'saved_department_id','job_id'], context=context)
            vals = {
                'active': False,
                'status': 'inactive',
                'job_id': data['job_id'][0],
            }
            if data['status'] and data['status'] == 'pending_inactive':
                vals.update(
                    {'department_id': data['saved_department_id'] and data['saved_department_id'][0] or False,
                     'saved_department_id': False})
 
            self.pool.get('hr.employee').write(
                cr, uid, data['id'], vals, context=context)
...
class hr_employee_termination(osv.osv):
    ...
    _columns = {
        ...
        'badge_id': fields.char("Badge ID", readonly=True, states={
                                         'draft': [('readonly', False)]}),
    ...
    _track = {
        'state': {
            ...
            'hr_employee_state.mt_alert_state_draft': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'draft',
        },
    }
    ...
    def state_draft(self, cr, uid, ids, context=None):
        emp_data_pool = self.pool.get("hr.employee")
        contract_data_pool = self.pool.get("hr.contract")
        for term in self.browse(cr, uid, ids, context=context):
#             if term.state not in ['done']:
#                 raise osv.except_osv(_('Unable to delete record!'),
#                                      _('Employment termination already in progress. Use the "Cancel" button instead.'))
            ...
            # to Open
            wkf = netsvc.LocalService('workflow')
            emp_data_pool.write(cr, uid, term.employee_id.id, {'status': 'active'}, context=context)
            emp_data_pool.write(cr, uid, term.employee_id.id, {'active': 1}, context=context)
            wkf.trg_validate(
                uid, 'hr.employee', term.employee_id.id, 'signal_active', cr)
#             for contract in term.employee_id.contract_ids:
#                 if contract.state in 'done,pending_done':
#                     contract_data_pool.write(cr, uid, contract.id, {'state': 'open'}, context=context)       
#                     wkf.trg_validate(
#                         uid, 'hr.contract', contract.id, 'signal_open', cr)
  
            self.write(cr, uid, term.id, {'state': 'draft'}, context=context)
...
#             contract_data_pool.write(cr, uid, contract.id, {'state': 'done'}, context=context)
#             wkf.trg_validate(
#                 uid, 'hr.contract', contract.id, 'signal_done', cr)
#             return
...
#             return
        ...
        wkf = netsvc.LocalService('workflow')
        contract_data_pool.write(cr, uid, contract.id, {'state': 'pending_done'}, context=context)
        wkf.trg_validate(
            uid, 'hr.contract', contract.id, 'signal_pending_done', cr)

        # Set employee state to pending deactivation
        emp_data_pool.write(cr, uid, contract.employee_id.id, {'status': 'pending_inactive'}, context=context)
        wkf.trg_validate(
...
class hr_employee_tax_history(osv.osv):

    _name = 'hr.employee.job.history'
    _description = 'Data for Employee Tax History'
