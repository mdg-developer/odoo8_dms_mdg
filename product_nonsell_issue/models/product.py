...
class product_nonsell_issue_line(osv.osv):
    ...
    def on_change_product_id(self, cr, uid, ids, product_id, pricelist_id, context=None):
        ...
        if product_id and pricelist_id:
            ...
            cr.execute("select new_price from product_pricelist_item where price_version_id in ( select id from product_pricelist_version where pricelist_id=%s and active=True) and product_id=%s and product_uom_id=%s", (pricelist_id, product.id, product.product_tmpl_id.uom_id.id,))
