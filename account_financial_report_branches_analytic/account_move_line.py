...
class account_move_line(osv.osv):
    ...
    def _query_get(self, cr, uid, obj='l', context=None):
        ...
        if context.get('account_analytic_id',False):
            analytic_id = context['account_analytic_id']
            print 'analytic >>>> ',analytic_id
            #print "account analytic_id %s", analytic_id
            result = {}
            child_ids=[]
            analytic_query=None
#             analytic_id=str(analytic_id).split(',')[0].replace('[','').replace(']','')
            try:
