...
class branch_stock_requisition(osv.osv):
    ...
    def on_change_branch_id(self, cr, uid,ids,branch_id, context=None):
        ...
        if branch_id:
            ...
            branch_location = branch_data.branch_location_id
            if branch_location:
                location=branch_location.id
            else:
                raise osv.except_osv(_('Warning!'), _('Please Insert Branch Location In Branch Form!'))
    ...
    _columns = {
        ...
        'to_location_id':fields.many2one('stock.location', 'Request Warehouse' , required=True),
   ...
   # 'good_issue_id':fields.many2many('branch.good.issue.note', 'GIN No' , readonly=True),
   ...
   'pricelist_id': fields.many2one('product.pricelist', 'Price list', required=True),
    ...
    def create(self, cursor, user, vals, context=None):
        if vals['branch_id']:
            ...
            branch_data = self.pool.get('res.branch').browse(cursor, user, branch_id, context=context)
            from_location_id = branch_data.branch_location_id.id            
