...
class hr_transfer(osv.Model):
    ...
    _columns = {
        ...
        'src_id': fields.many2one('hr.job', 'From Job', readonly=True, required=True,states={'draft': [('readonly', False)]}),
        
        'dst_id': fields.many2one('hr.job', 'Destination Job', required=True, readonly=True,states={'draft': [('readonly', False)]}),
        
        'src_department_id': fields.many2one('hr.department','From Department', required=True, readonly=True,states={'draft': [('readonly', False)]}),
        
        'dst_department_id': fields.many2one('hr.department','Destination Department', required=True),
        
        'src_contract_id': fields.many2one('hr.contract', 'From Contract', readonly=True,states={'draft': [('readonly', False)]}),
#        'dst_contract_id': fields.many2one('hr.contract', 'Destination Contract', readonly=True),
    ...
    def state_done(self, cr, uid, ids, context=None):
        ...
        for xfer in self.browse(cr, uid, ids, context=context):
            ...
            employee_obj.write(
                    cr, uid, xfer.employee_id.id, {
                        'department_id': xfer.dst_department_id.id,'job_id':xfer.dst_id.id,'section_id':xfer.to_section_id.id},
                    context=context)
            contract_obj.write(cr, uid, xfer.src_contract_id.id, {'job_id':xfer.dst_id.id}, context=context)        
