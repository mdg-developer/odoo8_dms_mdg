...
class account_voucher(osv.osv):
    ...
    def voucher_move_line_create(self, cr, uid, voucher_id, line_total, move_id, company_currency, current_currency, context=None):
        ...
        for line in voucher.line_ids:
            if line.total_discount > 0:
                ...
                move_line = {
                    ...
                    'quantity': 1,
                    'credit': 0.0,
                    'debit':line.total_discount,
                    'date': voucher.date
                }
                voucher_line=move_line_obj.create(cr, uid, move_line)
                move_line = {}
                rec_ids = [voucher_line, line.move_line_id.id]
                print 'line by line discount >>>>', line.total_discount
        ...
        if total_discount > 0:
            ...
            voucher_line=move_line_obj.create(cr, uid, move_line_total)
            move_line = {}
            rec_ids = [voucher_line, line.move_line_id.id]
            print 'Total discount >>>>', total_discount
        return (tot_line, rec_lst_ids)
    
    def onchange_discount_account_id(self, cr, uid, ids, discount_account_id, context=None):
        res = super(account_voucher, self).onchange_discount_account_id(cr, uid, ids, discount_account_id, context=context)
        if discount_account_id:
            account = self.pool.get('account.account').browse(cr, uid, discount_account_id, context=context)
            res['value'].update({'discount_account_id': account.discount_account_id and account.discount_account_id.id or False, })
        else: 
...
class account_voucher_line(osv.osv):
    ...
    def onchange_discount_amount(self, cr, uid, ids, total_discount, context=None):
        vals = {}
        if total_discount:
