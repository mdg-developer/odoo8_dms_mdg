-- Function: financial_report_monthly_balance(integer, integer, integer, text, integer)

-- DROP FUNCTION financial_report_monthly_balance(integer, integer, integer, text, integer);

CREATE OR REPLACE FUNCTION financial_report_monthly_balance(report_id integer, fiscal_year integer, period_id integer, state_cond text, branch integer)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
  ...
  BEGIN
    ...
    EXECUTE sql into balance USING account_ids,fiscal_year,period_id,state_cond, branch;
    IF balance < 0 THEN
    balance = balance;
    END IF;
  ...
  COST 100;
ALTER FUNCTION financial_report_monthly_balance(integer, integer, integer, text, integer)
  OWNER TO odoo;
