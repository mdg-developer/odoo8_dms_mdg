...
# 
#         return res
    def onchange_start(self, cr, uid, ids, date_start,employee_id, context=None):

        values = {}
        if date_start:
            trial_date_end = datetime.strptime(date_start,'%Y-%m-%d').date()+ relativedelta(months=+3)
            date_end = datetime.strptime(date_start,'%Y-%m-%d').date()+ relativedelta(months=+24)
            values = {
                'trial_date_start' : date_start,
                'trial_date_end' :trial_date_end.strftime(DEFAULT_SERVER_DATE_FORMAT),
...
#             if date_start != emp_obj.initial_employment_date:
#                 raise osv.except_osv(_('Duration Date mismatch!'),
#                                      _('Duration Start Date must be the same with Initial Employment Date.\nEmployee: %s')% emp_obj.name)
    ...
    def state_done(self, cr, uid, ids, context=None):
        ...
        for i in ids:
            ...
            print 'datalllllllllllllllll',data, data['job_id'][0]
            vals = {'state': 'done',
                    'date_end': False,
                    'job_id': data['job_id'][0],
                    'end_job_id': data['job_id'][0]}
            print 'valslllllllllllllllll',vals
