...
CREATE OR REPLACE VIEW public.stock_journal
 ...
 SELECT branch.name AS branch,
    ...
    region.name,
    loc_group.name AS location_group,
   ...
   FROM stock_quant sq,
    ...
    product_maingroup principal,
    stock_location_group loc_group
  WHERE sq.product_id = pp.id AND pp.product_tmpl_id = pt.id AND pt.report_uom_id = uom.id AND sq.location_id = loc.id AND loc.branch_id = branch.id AND branch.branch_region_id = region.id AND pt.categ_id = category.id AND pt.main_group = principal.id AND loc.stock_location_group_id = loc_group.id
  GROUP BY pp.name_template, uom.factor, sq.in_date, loc.name, branch.name, category.name, principal.name, region.name, loc_group.name
