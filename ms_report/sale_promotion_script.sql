...
CREATE OR REPLACE VIEW promotion_sale_temp_report AS 
 SELECT DISTINCT (aa.code::text || '-'::text) || aa.name::text AS account_name,
    ...
    invoice_line.price_subtotal::double precision + invoice_line.discount_amt AS sub_total,
    ( SELECT promos_rules.description
           FROM promos_rules
          WHERE promos_rules.id = promodata.pro_id) AS promotion,
    promodata.pro_id AS promotion_id,
    ai.branch_id,
    ai.date_invoice,
   ...
   FROM sale_order so,
    ...
    product_uom uom,
    product_template pt
  WHERE so.name::text = ai.origin::text AND pt.id = product.product_tmpl_id AND ai.id = invoice_line.invoice_id AND promodata.name::text = so.tb_ref_no::text AND invoice_line.product_id = promodata.product_id AND aa.id = invoice_line.account_id AND so.partner_id = partner.id AND so.section_id = sale_team.id AND invoice_line.product_id = product.id AND invoice_line.uos_id = uom.id
  ORDER BY ai.number;

ALTER TABLE promotion_sale_temp_report
  OWNER TO odoo;
