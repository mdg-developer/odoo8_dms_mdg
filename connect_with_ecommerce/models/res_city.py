...
class res_city(osv.osv):
    ...
    def get_all_stock_status(self, cr, uid, ids, city_name=None, township_name=None, woo_customer_id=None, context=None):
    ...
    def get_stock_status_label_multi_customer(self, cr, uid, ids, woo_customer_id=None, context=None):

        partner_obj = self.pool.get('res.partner')
        location_ids = []
        data = []
        if woo_customer_id:
            for customer in woo_customer_id:
                woo_customer = '1_' + customer
                partner_data = partner_obj.search(cr, uid, [('woo_customer_id', '=', woo_customer)], context=context)
                if partner_data:
                    partner_value = partner_obj.browse(cr, uid, partner_data, context=context)
                    branch_id = partner_value.township.delivery_team_id.branch_id.id if partner_value.township.delivery_team_id.branch_id else None
                    if branch_id:
                        ...
                        if location_record:
                            ...
                            if location_ids:
                                ...
                                product_record = cr.dictfetchall()
                                if product_record:
                                    data.append({"woo_customer_id": customer,
                                                 "data": product_record})
            return data