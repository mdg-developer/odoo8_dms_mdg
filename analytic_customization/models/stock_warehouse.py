    ...
    def _get_invoice_line_vals(self, cr, uid, move, partner, inv_type, context=None):
        ...
        if inv_type in ('out_invoice', 'out_refund'):
            if inv_type in ('out_refund'):
                if move.product_id.categ_id.property_sale_return_account_id.id: 
                    account_id = move.product_id.categ_id.property_sale_return_account_id.id 
                else:
                    raise except_orm(_('Sale Return Account Error!'),
                                        _('Product %s has no related Sale Return Account.') % (move.product_id.name))                        
            else:  
        ...
        return {
            ...
            'discount': 0.0,
            'account_analytic_id': move.product_id.product_tmpl_id.categ_id.analytic_account_id.id or False,
        }
