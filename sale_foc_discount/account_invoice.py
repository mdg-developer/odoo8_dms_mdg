...
class account_invoice(models.Model):
    ...
    def _compute_amount(self):
        for inv in self:
            inv.amount_untaxed = sum(line.price_subtotal for line in inv.invoice_line)
            inv.amount_tax = sum(line.amount for line in inv.tax_line)
            total_discount_amt = sum(line.discount_amt for line in inv.invoice_line) - sum(line.price_unit for line in inv.invoice_line if line.price_subtotal<0)
            inv.discount_total = total_discount_amt
            inv.amount_total = inv.amount_untaxed + inv.amount_tax - inv.deduct_amt - (inv.amount_untaxed * (inv.additional_discount / 100))
