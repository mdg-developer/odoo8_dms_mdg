...
#             self.create_attendance_new_csv(cr, uid,lines,context=ctx)       
    ...
    def create_attendance_new(self, cr, uid, lines, context):
        ...
        for attance_line in attendances:
            try:
                entry_date = attance_line['aDate']
                #print 'this is entry date',entry_date
...
#Excel
    ...
    #Excel Impor
    def prepared_attendance_excel_12hours(self,fields,input_tz,attendances):
        #print 'fields',fields
        if len(fields)>29:
            return;
...
#Excel import
    def prepared_attendance_excel_24hours(self, fields, input_tz, attendances):
        print 'fields', fields
        if len(fields) > 29:
            return;
        time_in = time_out = False
...
#         local_date_entry = datetime.strptime(a, '%Y-%m-%d %H:%M:%S')
#         dt = input_tz.localize(local_date_entry, is_dst=False)
#     # And convert to UTC before reformatting for writing
#         utc_date_entry = dt.astimezone(pytz.UTC)
...
#         time_entry = utc_date_entry.strftime("%H:%M:%S")
        ...
        result = xlrd.xldate.xldate_as_tuple(day_time, 0)
        a=str(result[0]) + '-' + str(result[1]) + '-' + str(result[2]) + ' ' + str(result[3]) + ':' + str(result[4]) + ':' + str(result[5])
        day_1 = datetime.strptime(a, '%Y-%m-%d %H:%M:%S')
        in_day = day_1.date()
        emp_id = int(fields[0])
        emp_name = fields[2]
        date_time = fields[1]
        ...
        a = str(result[0]) + '-' + str(result[1]) + '-' + str(result[2]) + ' ' + str(result[3]) + ':' + str(result[4]) + ':' + str(result[5])
        date_data = str(datetime.strptime(a, '%Y-%m-%d %H:%M:%S'))
        in_time = int(result[3])
        ...
        if in_time < 12:
            ...
            local_date_entry = None
            try:
                local_date_entry = datetime.strptime(date_time_in, '%d-%b-%y :%M:%S')
            except Exception, e:
                if date_time_in:
    ...
    def prepared_name_attendance(self, fields, input_tz, attendances):
        ...
        action_desc = ""
        time=fields[4]
        local_date_entry = datetime.strptime(time, '%m/%d/%Y %H:%M:%S PM')
        dt = input_tz.localize(local_date_entry, is_dst=False)
    # And convert to UTC before reformatting for writing
        utc_date_entry=dt.astimezone(pytz.UTC)
        utc_date_entry_str = dt.astimezone(pytz.UTC).strftime('%m/%d/%y %H:%M:%S')
        date_only = utc_date_entry.strftime("%m/%d/%y")
        time_entry = utc_date_entry.strftime("%H:%M:%S")
        vals = {}
    ...
    def prepared_wihtoutname_attendance(self, fields, input_tz, attendances):
        action_desc = "";
        local_date_entry = datetime.strptime(fields[1], '%m/%d/%Y %H:%M:%S')
        dt = input_tz.localize(local_date_entry, is_dst=False)
    # And convert to UTC before reformatting for writing
        utc_date_entry=dt.astimezone(pytz.UTC)
        utc_date_entry_str = dt.astimezone(pytz.UTC).strftime('%m/%d/%Y %H:%M:%S')
    ...
    def prepared_attendance(self, cr,uid,ids,lines,excel_import_file,context=None):
        attendances = []
        input_tz = self._input_tz(cr, uid, context)# Apply input tz to the parsed naive datetime
        for line in lines:
            if excel_import_file and line:
                self.prepared_attendance_excel_24hours(line,input_tz,attendances)
            else:
                fields = line.split('\t')
                if len(fields) == 5:
                    self.prepared_name_attendance(fields,input_tz,attendances)
...
#Excel Import    
    ...
    def create_att_new(self, cr, uid,ids, lines,excel_import_file, context):
        ...
        #print 'this is to_ date',to_date
        #print 'this is fromt date',from_date
        for emp in hr_emp_data:
            if emp['fingerprint_id']:
                emp_fingerprint[int(emp['fingerprint_id'])] = emp['id']  
        attendances = self.prepared_attendance(cr, uid,ids, lines,excel_import_file, context)
        #print "Total Number of attendance records %d" % len(attendances)
        ...
        if attendances:   
            for att_line in attendances:
                try:
                    emp_name=att_line['Name']
                    emp_id=att_line['id']
                    entry_date=att_line['date']
                    if emp_name:
                        employee_code=str(emp_name)
                        emp_rar=employee_pool .search(cr,uid,[('name_related','=',employee_code)])
                        if not emp_rar:
                            print 'no employee'
                        else:
                            emp_id=emp_rar[0]
                    ...
                    if emp_id:
                        ...
                        break_in_time = None
                        break_out_time = None
                        if 'inTime' in att_line:
                            in_time = att_line['inTime']
                            print 'this is inTime',in_time
                            
                        if 'outTime' in att_line:
                            out_time = att_line['outTime']
                        
                        if 'Breakout' in att_line:
                            break_out_time = att_line['Breakout']
                            
                        if 'Breakin' in att_line:
                            break_in_time = att_line['Breakin']
                        
                        if employee_id and from_date<=entry_date<=to_date:# a little fix
    ...
    def prepared_name_attendance_csv(self, cr,uid,amls, input_tz, attendances,context=None):
        ...
        for aml in amls:
            ...
            if time:
                if time.find('AM') !=-1:
                    local_date_entry = datetime.strptime(time, '%m/%d/%Y %H:%M:%S AM')
                    dt = input_tz.localize(local_date_entry, is_dst=False)
            # And convert to UTC before reformatting for writing
                    utc_date_entry=dt.astimezone(pytz.UTC)
                    utc_date_entry_str = dt.astimezone(pytz.UTC).strftime('%m/%d/%y %H:%M:%S AM')
            ...
            # And convert to UTC before reformatting for writing
                    ...
                    utc_date_entry_str = dt.astimezone(pytz.UTC).strftime('%m/%d/%y %H:%M:%S PM')
                    date_only = utc_date_entry.strftime("%m/%d/%y")
                    time_out = local_date_entry.strftime("%H:%M:%S")
                    time_out_out = datetime.strptime(time_out, "%H:%M:%S").strftime("%H:%M:%S")
                    #print 'date_only',date_only,' and time_entry',time_out
                    if time_out:
                        att_id = hr_att_obj.search(cr,uid,['&',('employee_id','=',emp_ids),('name','=',date_only+' '+time_out_out),('action','=','sign_out')])
                        if not att_id:
                            att_value = {'employee_id':emp_ids,'name':date_only+' '+str(time_out_out),'action':'sign_out'}
                            att_ids = hr_att_obj.create(cr,uid,att_value,context=context)
                            cr.commit()
                        else:
                            att_ids =att_id[0]                    
