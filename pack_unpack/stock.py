...
class StockMove(osv.osv):
    ...
    _columns = {
        'packing_id': fields.many2one('product.packing', 'Packing Order for Produced Products', select=True, copy=False),
        'pack_unpack_id': fields.many2one('product.packing', 'Production Order for Raw Materials', select=True),
        'unpacking_id': fields.many2one('product.unpacking', 'Packing Order for Produced Products', select=True, copy=False),
        'unpack_unpack_id': fields.many2one('product.unpacking', 'Production Order for Raw Materials', select=True),
        'used_for': fields.many2one('stock.move', 'Consumed for', help='Technical field used to make the traceability of produced products'),
        'unpack_used_for': fields.many2one('stock.move', 'Consumed for', help='Technical field used to make the traceability of produced products'),
    }
...
#             else:
    ...
    def action_consume_unpack(self, cr, uid, ids,produce_product, product_qty, location_id=False, restrict_lot_id=False, restrict_partner_id=False,
        ...
        if context is None:
            context = {}
        res = []
        production_obj = self.pool.get('product.unpacking')
        ...
        for move in self.browse(cr, uid, ids, context=context):
            if move.state == 'draft':
                ...
                ids2.append(move.id)
        print ' I am in the unpack action consume'
        prod_orders = set()
        for move in self.browse(cr, uid, ids2, context=context):
            print 'move',move
            prod_orders.add(move.unpack_unpack_id.id or move.unpacking_id.id)
    ...
    def action_consume(self, cr, uid, ids, product_qty, location_id=False, restrict_lot_id=False, restrict_partner_id=False,
        ...
        if context is None:
            context = {}
        res = []
        production_obj = self.pool.get('product.packing')
        ...
        prod_orders = set()
        for move in self.browse(cr, uid, ids2, context=context):
            prod_orders.add(move.pack_unpack_id.id or move.packing_id.id)
            move_qty = move.product_qty
            if move_qty <= 0:
                raise osv.except_osv(_('Error!'), _('Cannot consume a move with negative or zero quantity.'))
            quantity_rest = move_qty - product_qty
        ...
        if prod_orders:
            production_obj.signal_workflow(cr, uid, list(prod_orders), 'button_produce')
        return res
    def action_scrap_unpack(self, cr, uid, ids, product_qty, location_id, restrict_lot_id=False, restrict_partner_id=False, context=None):
        ...
        production_obj = self.pool.get('product.unpacking')
        for move in self.browse(cr, uid, ids, context=context):
            new_moves = super(StockMove, self).action_scrap_unpack(cr, uid, [move.id], product_qty, location_id,
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.get('state') == 'assigned':
            ...
            orders = list(set([x.pack_unpack_id.id for x in moves if x.pack_unpack_id and x.pack_unpack_id.state == 'confirmed']))
            for order_id in orders:
                if self.pool.get('product.packing').test_ready(cr, uid, [order_id]):
                    workflow.trg_validate(uid, 'product.packing', order_id, 'moves_ready', cr)
        return res
...
class stock_warehouse(osv.osv):
    ...
    def change_route(self, cr, uid, ids, warehouse, new_reception_step=False, new_delivery_step=False, context=None):
        ...
        if warehouse.in_type_id.default_location_dest_id != warehouse.manufacture_pull_id.location_id:
            self.pool.get('procurement.rule').write(cr, uid, warehouse.manufacture_pull_id.id, {'location_id': warehouse.in_type_id.default_location_dest_id.id}, context=context)
        return res
