...
class product_packing(osv.osv):
    ...
    def _from_move(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            res += self.pool.get("product.packing").search(cr, uid, [('move_lines', 'in', move.id)], context=context)
        return res
    ...
    _columns = {
        ...
        'move_lines': fields.one2many('stock.move', 'pack_unpack_id', 'Products to Consume',
            domain=[('state', 'not in', ('done', 'cancel'))], readonly=True, states={'draft': [('readonly', False)]}),
        'move_lines2': fields.one2many('stock.move', 'pack_unpack_id', 'Consumed Products',
            domain=[('state', 'in', ('done', 'cancel'))], readonly=True),
        'move_created_ids': fields.one2many('stock.move', 'packing_id', 'Products to Produce',
            domain=[('state', 'not in', ('done', 'cancel'))], readonly=True),
        'move_created_ids2': fields.one2many('stock.move', 'packing_id', 'Produced Products',
            domain=[('state', 'in', ('done', 'cancel'))], readonly=True),
        'product_lines': fields.one2many('product.packing.line', 'production_id', 'Scheduled goods',
            readonly=True),
    ...
    _defaults = {
        ...
        'name': lambda x, y, z, c: x.pool.get('ir.sequence').get(y, z, 'product.packing') or '/',
        'company_id': lambda self, cr, uid, c: self.pool.get('res.company')._company_default_get(cr, uid, 'product.packing', context=c),
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(product_packing, self).unlink(cr, uid, ids, context=context)
...
#                 'routing_id': False
    ...
    def _action_compute_lines(self, cr, uid, ids, properties=None, context=None):
        ...
        bom_obj = self.pool.get('bom.product')
        uom_obj = self.pool.get('product.uom')
        prod_line_obj = self.pool.get('product.packing.line')
...
#                 workcenter_line_obj.create(cr, uid, line)
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        proc_obj = self.pool.get("procurement.order")
        procs = proc_obj.search(cr, uid, [('packing_id', 'in', ids)], context=context)
        if procs:
...
#####finished
            for produce_product in production.move_created_ids:
                ...
                stock_mov_obj.write(cr, uid, new_moves, {'packing_id': production_id}, context=context)
                ...
                if remaining_qty: # In case you need to make more than planned
                    ...
                    extra_move_id = stock_mov_obj.copy(cr, uid, produce_product.id, default={'state': 'confirmed',
                                                                                             ...
                                                                                             'packing_id': production_id}, context=context)
                    if extra_move_id:
    ...
    def action_assign(self, cr, uid, ids, context=None):
        ...
        for production in self.browse(cr, uid, ids, context=context):
            ...
            if self.pool.get('product.packing').test_ready(cr, uid, [production.id]):
                workflow.trg_validate(uid, 'product.packing', production.id, 'moves_ready', cr)
    ...
    def force_production(self, cr, uid, ids, *args):
        ...
        for order in self.browse(cr, uid, ids):
            ...
            if self.pool.get('product.packing').test_ready(cr, uid, [order.id]):
                workflow.trg_validate(uid, 'product.packing', order.id, 'moves_ready', cr)
        return True

     
product_packing()

class product_packing_line(osv.osv):
    ...
    _columns = {
        ...
        'production_id': fields.many2one('product.packing', 'Production Order', select=True),
    }

product_packing_line()
