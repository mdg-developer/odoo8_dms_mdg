...
class pack_product_produce_line(osv.osv_memory):
    _name="pack.product.produce.line"
    ...
    _columns = {
        ...
        'produce_id': fields.many2one('pack.product.produce','Produce ID'),
        'tracking_production': fields.related('product_id', 'tracking_production', type='boolean'),
    }
pack_product_produce_line()

class pack_product_produce(osv.osv_memory):
    _name = "pack.product.produce"
    ...
    _columns = {
        ...
        'consume_lines': fields.one2many('pack.product.produce.line', 'produce_id', 'Products Consumed'),
    ...
    def on_change_qty(self, cr, uid, ids, product_qty, consume_lines, context=None):
        ...
        """
        prod_obj = self.pool.get("product.packing")
    ...
    def _get_product_qty(self, cr, uid, context=None):
        ...
        if context is None:
            context = {}
        prod = self.pool.get('product.packing').browse(cr, uid,
    ...
    def do_produce(self, cr, uid, ids, context=None):
        ...
        self.pool.get('product.packing').action_produce(cr, uid, production_id,
                            data.product_qty, data.mode, data, context=context)
        return {}

pack_product_produce()
