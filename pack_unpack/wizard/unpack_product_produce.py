...
class unpack_product_produce_line(osv.osv_memory):
    _name="unpack.product.produce.line"
    ...
    _columns = {
        ...
        'produce_id': fields.many2one('unpack.product.produce','Produce ID'),
        'tracking_production': fields.related('product_id', 'tracking_production', type='boolean'),
    }
unpack_product_produce_line()

class unpack_product_produce(osv.osv_memory):
    _name = "unpack.product.produce"
    ...
    _columns = {
        ...
        'mode': fields.selection([('con_produce', 'Consume & Produce'),
                                  ('con_only', 'Consume Only')], 'Mode', required=True,
        ...
        'consume_lines': fields.one2many('unpack.product.produce.line', 'produce_id', 'Products Consumed'),
        'tracking_production': fields.boolean('Track production'),
    }
    ...
    def on_change_qty(self, cr, uid, ids, product_qty, consume_lines, context=None):
        ...
        """
        prod_obj = self.pool.get("product.unpacking")
        ...
        if product_qty > 0.0:
            ...
            print 'consume_lines',consume_lines
    ...
    def _get_product_qty(self, cr, uid, context=None):
        ...
        if context is None:
            context = {}
        prod = self.pool.get('product.unpacking').browse(cr, uid,
    ...
    def _get_track(self, cr, uid, context=None):
        ...
        return prod and prod_obj.browse(cr, uid, prod, context=context).tracking_production or False

    _defaults = {
         'product_qty': _get_product_qty,
         'mode': lambda *x: 'con_produce',
         'product_id': _get_product_id,
         'tracking_production': _get_track, 
    }
    ...
    def do_produce(self, cr, uid, ids, context=None):
        ...
        self.pool.get('product.unpacking').action_produce(cr, uid, production_id,
                            data.product_qty, data.mode, data, context=context)
        return {}

unpack_product_produce()
