# -*- coding: utf-8 -*-
##############################################################################
# For copyright and license notices, see __openerp__.py file in module root
# directory
##############################################################################
from openerp import models, fields, api


class sale_order_add_multiple(models.TransientModel):
    _name = 'stock.requisition.add_multiple'
    _description = 'Stock Requisition add multiple'

    quantity = fields.Float('Quantity',
                            default='1.0')
    products_ids = fields.Many2many(
        'product.product',
        string='Products',
        #domain=[('sale_ok', '=', True)],
    )

    @api.one
    def add_multiple(self):
        active_id = self._context['active_id']
        sale = self.env['stock.requisition'].browse(active_id)
        for product_id in self.products_ids:
#             product = self.env['sale.order.line'].product_id_change(
#                 sale.pricelist_id.id,
#                 product_id.id,
#                 qty=self.quantity,
#                 uom=product_id.uom_id.id,
#                 partner_id=sale.partner_id.id)
            val = {
                #'name': product['value'].get('name'),
                'product_quantity': self.quantity,
                'line_id': active_id,
                'product_id': product_id.id or False,
                'product_uos': product_id.uom_id.id,
                #'price_unit': product['value'].get('price_unit'),
                #'tax_id': [(6, 0, product['value'].get('tax_id'))],
            }
            self.env['stock.requisition.line'].create(val)
