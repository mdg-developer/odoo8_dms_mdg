...
class stock_taking_loading_instruction(osv.osv):
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    _name = "stock.taking.loading.instruction"
    _description = "Stock Taking and Loading Instruction"
    _order = "id desc"    
#     _track = {
#         'state': {
...
#     }    
    ...
    _columns = {
        'name': fields.char('(STLI) Ref:No.', readonly=True),
        ...
        'operation_date':fields.datetime('Date of Operation'),
        'request_gin_id':fields.many2one('issue.requisition', 'GIN Ref.', readonly=True),
...
#                but waiting for the scheduler to run on the order date.", select=True),
      'p_line':fields.one2many('stock.taking.loading.instruction.line', 'line_id', 'Product Lines',
                              copy=True),
    ...
    def create(self, cursor, user, vals, context=None):
        id_code = self.pool.get('ir.sequence').get(cursor, user,
                                                'stock.taking.loading.instruction.code') or '/'
        vals['name'] = id_code
        return super(stock_taking_loading_instruction, self).create(cursor, user, vals, context=context)
    
    def cancel(self, cr, uid, ids, context=None):
        return self.write(cr, uid, ids, {'state': 'cancel', })
    
    def confirm(self, cr, uid, ids, context=None):
        product_line_obj = self.pool.get('stock.taking.loading.instruction.line')
        note_obj = self.pool.get('stock.taking.loading.instruction')
...
class stock_taking_loading_instruction_line(osv.osv):  # #prod_pricelist_update_line
    _name = 'stock.taking.loading.instruction.line'
    ...
    _columns = {                
        ...
        'qty_in_ctn':fields.integer('Qty in Ctn'),
