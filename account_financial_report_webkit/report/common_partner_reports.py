...
class CommonPartnersReportHeaderWebkit(CommonReportHeaderWebkit):
    ...
    def get_partners_move_lines_ids(self, account_id, analytic_account_ids, main_filter, start, stop,
                                    target_move,branch_id,
        ...
        if filter_from:
            return self._get_partners_move_line_ids(
                filter_from, account_id, analytic_account_ids, start, stop, target_move,branch_id,
    ...
    def _get_partners_move_line_ids(self, filter_from, account_id, analytic_account_ids, start, stop,
                                    target_move,branch_id,
...
#             search_params.update({'branch_id': branch_id[0]})
    ...
    def _partners_initial_balance_line_ids(self, account_ids, analytic_account_ids, start_period,branch_id,
                                           partner_filter,
                                           exclude_reconcile=False,
        ...
        if branch_id:
            search_param.update({'branch_id': tuple(branch_id)})
            sql += ("AND ml.branch_id in %(branch_id)s")     
#         if branch_id:
#             sql += "AND ml.branch_id =  %(branch_id)s "
#             search_param.update({'branch_id': branch_id[0]})        
        self.cursor.execute(sql, search_param)
        return self.cursor.dictfetchall()

    def _compute_partners_initial_balances(self,account_ids, analytic_account_ids, start_period,branch_id,
                                           partner_filter=None,
                                           exclude_reconcile=False,
                                           force_period_ids=False):
        ...
        move_line_ids = self._partners_initial_balance_line_ids(account_ids, analytic_account_ids, start_period,branch_id, partner_filter,
            exclude_reconcile=exclude_reconcile,
