...
class CommonReportHeaderWebkit(common_report_header):
    ...
    def _get_branch(self, data):
        ...
        #return self._get_form_param('branch_id', data)
    ...
    ###############################
    def _compute_init_balance_new(self, account_id=None, analytic_id=None, branch_id=None, period_ids=None,
            ...
            try:
                if analytic_id and branch_id:                    
                    self.cursor.execute("SELECT sum(debit) AS debit, "
                                        ...
                                        " AND branch_id in %s",                                        
                                        (tuple(period_ids), account_id, tuple(analytic_id),tuple(branch_id)))
                elif analytic_id:
                    self.cursor.execute("SELECT sum(debit) AS debit, "
                                        " sum(credit) AS credit, "
                                        " sum(debit)-sum(credit) AS balance, "
                                        ...
                                        " AND analytic_account_id in %s"
                                        ,                                        
                                        (tuple(period_ids), account_id, tuple(analytic_id)))
                elif branch_id:
                    self.cursor.execute("SELECT sum(debit) AS debit, "
                                        " sum(credit) AS credit, "
                                        " sum(debit)-sum(credit) AS balance, "
                                        ...
                                        " AND account_id = %s"                                       
                                        " AND branch_id in %s",                                        
                                        (tuple(period_ids), account_id,tuple(branch_id)))        
                else:
                    self.cursor.execute("SELECT sum(debit) AS debit, "
                                        " sum(credit) AS credit, "
                                        " sum(debit)-sum(credit) AS balance, "
    ...
    def _compute_init_balance(self, account_id=None, analytic_id=None, branch_id=None, period_ids=None,
                              mode='computed', default_values=False):
        if not isinstance(period_ids, list):
            period_ids = [period_ids]
        res = {}        
    ...
    def _compute_init_balance_by_date(self, account_id=None, analytic_id=None, branch_id=None, start=None, target_move=None,
                                      mode='computed', default_values=False):        
        res = {}
    ...
    def _read_opening_balance_new(self, account_ids, new_analytic_account_ids, branch_ids, start_period):
        ...
        res = {}
        for account_id in account_ids:
            res[account_id] = self._compute_init_balance_new(
                account_id, new_analytic_account_ids, branch_ids, opening_period_selected, mode='read')
        return res
    
    def _read_opening_balance(self, account_ids, new_analytic_account_ids, branch_ids, start_period):
        """ Read opening balances from the opening balance
        """
        opening_period_selected = self.get_included_opening_period(
            start_period)
    ...
    def _read_opening_balance_by_date(self, account_ids, new_analytic_account_ids, branch_ids, start, target_move):
        """ Read opening balances from the opening balance
        """        
        res = {}
        for account_id in account_ids:
            res[account_id] = self._compute_init_balance_by_date(
                account_id, new_analytic_account_ids,branch_ids, start, target_move, mode='read')
        return res
            ...
            if acc.user_type.close_method == 'none':
                ...
                if pnl_periods_ids and not opening_period_selected:
                    res[acc.id] = self._compute_init_balance(
                        acc.id,new_analytic_account_ids,branch_ids, pnl_periods_ids)
            else:
                res[acc.id] = self._compute_init_balance(acc.id,new_analytic_account_ids,branch_ids, bs_period_ids)
        return res

    ################################################
    # Account move retrieval helper                #
    ################################################
    def _get_move_ids_from_periods(self, account_id, new_analytic_account_ids, period_start, period_stop,
                                   target_move,branch):
    ...
    def _get_move_ids_from_dates(self, account_id, new_analytic_account_ids, date_start, date_stop,
        ...
        if new_analytic_account_ids:
            search_period = [('date', '>=', date_start),
                         ('date', '<=', date_stop),
                         ('account_id', '=', account_id),
                         ('analytic_account_id', 'in', new_analytic_account_ids)]
    ...
    def get_move_lines_ids(self, account_id, new_analytic_account_ids, main_filter, start, stop,
        ...
        if main_filter in ('filter_period', 'filter_no'):
            return self._get_move_ids_from_periods(account_id, new_analytic_account_ids, start, stop,
                                                   target_move,branch)

        elif main_filter == 'filter_date':
            return self._get_move_ids_from_dates(account_id, new_analytic_account_ids, start, stop,
                                                 target_move,branch)
        else:
            raise osv.except_osv(
...
SELECT account_move.id,
       array_to_string(
          ARRAY(SELECT DISTINCT a.code || '-'|| a.name
...
WHERE move_id in %s"""
    ...
    def _get_amount_currency_mode(self, data):
        currbal= self._compute_init_balance(default_values=True)
        if currbal:
