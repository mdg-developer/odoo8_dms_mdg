-- Function: financial_report_monthly_balance_analytic(integer, integer, integer, text, integer, integer)

-- DROP FUNCTION financial_report_monthly_balance_analytic(integer, integer, integer, text, integer, integer);

CREATE OR REPLACE FUNCTION financial_report_monthly_balance_analytic(report_id integer, fiscal_year integer, period_id integer, state_cond text, analytic_id integer, branch integer)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
...
EXECUTE sql into balance USING analytic_account_ids,fiscal_year,period_id,state_cond,branch;
IF balance < 0 THEN
	balance = balance * -1;
END IF;
  ...
  COST 100;
ALTER FUNCTION financial_report_monthly_balance_analytic(integer, integer, integer, text, integer, integer)
  OWNER TO odoo;
