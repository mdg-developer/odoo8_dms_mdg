...
CREATE OR REPLACE VIEW financial_report_tree AS 
 ...
 --financial_report_analytics
  
CREATE OR REPLACE VIEW financial_report_analytic_tree AS 
 WITH RECURSIVE report_tree(id, account_analytic_id,name, sign, style_overwrite, sequence, depth, path, spacer) AS (
         SELECT tn.id,tn.account_analytic_id,
            tn.name,
            tn.sign,
            tn.style_overwrite,
            tn.sequence,
          ...
          WHERE tn.parent_id IS NULL
        UNION ALL
         SELECT c.id,c.account_analytic_id,
            (p.spacer || '    '::text) || c.name::text,
            c.sign,
            c.style_overwrite,
            c.sequence,
 ...
 SELECT row_number() OVER (ORDER BY n.id) AS row_number,
	    n.path[1] AS parent_id,
	    n.id,
	    n.account_analytic_id,
	    n.name,
	    n.sign,
	    n.style_overwrite,
	    n.sequence,
...
$BODY$
  ...
  BEGIN
	...
	END IF;
        ...
        --IF period_ids IS NOT NULL THEN
            -- sql := sql || ' AND aml.period_id in ('|| array_to_string(period_ids, ',') ||')';
	--END IF;
...
-- Function: financial_report_balance_net(integer, integer, date, date, integer, integer, text)

-- DROP FUNCTION financial_report_balance_net(integer, integer, date, date, integer, integer, text);

CREATE OR REPLACE FUNCTION financial_report_balance_net(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
  ...
  BEGIN
    ...
    RAISE NOTICE ' SQL (%)', sql;
    EXECUTE sql into balance USING account_ids,fiscal_year,start_date,end_date,start_period_id,end_period_id,state_cond;
    --EXECUTE sql into balance USING account_ids,fiscal_year,start_date,end_date,(start_period_id-1),state_cond;
    IF balance < 0 THEN
    balance = balance * -1;
    END IF;
        RAISE NOTICE 'balance (%)', balance;
...
$BODY$
  ...
  -- Function: financial_report_balance_ksamount(integer, integer, date, date, integer, integer, text)

-- DROP FUNCTION financial_report_balance_ksamount(integer, integer, date, date, integer, integer, text);

CREATE OR REPLACE FUNCTION financial_report_balance_ksamount(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
...
-- Function: financial_report_balance_usdamount(integer, integer, date, date, integer, integer, text)

-- DROP FUNCTION financial_report_balance_usdamount(integer, integer, date, date, integer, integer, text);

CREATE OR REPLACE FUNCTION financial_report_balance_usdamount(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
...
CREATE OR REPLACE FUNCTION financial_report_balance_opening(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
  ...
  BEGIN
    ...
    IF start_period_id >0  and end_period_id >0 and start_period_id-1 is not null and end_period_id is not null  THEN
      sql := sql || ' AND aml.period_id between ($5-1) and $6';
    END IF;
...
CREATE OR REPLACE FUNCTION financial_report_balance_purchase(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
...
-- Function: financial_report_monthly_ksbalance(integer, integer, integer, text)

-- DROP FUNCTION financial_report_monthly_ksbalance(integer, integer, integer, text);

CREATE OR REPLACE FUNCTION financial_report_monthly_ksbalance(report_id integer, fiscal_year integer, period_id integer, state_cond text)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
  ...
  -- Function: financial_report_monthly_usdbalance(integer, integer, integer, text)

-- DROP FUNCTION financial_report_monthly_usdbalance(integer, integer, integer, text);

CREATE OR REPLACE FUNCTION financial_report_monthly_usdbalance(report_id integer, fiscal_year integer, period_id integer, state_cond text)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
...
-- Function: financial_report_balance_analytic(integer, integer, date, date, integer, integer, text, text)

-- DROP FUNCTION financial_report_balance_analytic(integer, integer, date, date, integer, integer, text, text);

CREATE OR REPLACE FUNCTION financial_report_balance_analytic(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text, analytic_code text)
  RETURNS numeric AS
$BODY$
  DECLARE
balance numeric;
analytic_id int;
  BEGIN
	...
	select  id into analytic_id from account_analytic_account where code = analytic_code LIMIT 1;
	
    END IF;
    select  * into balance from financial_report_balance_analytic(report_id,fiscal_year,start_date,end_date,start_period_id, end_period_id,state_cond,analytic_id);
  END;
  $BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
...
-- Function: financial_report_balance_ksamount_negative(integer, integer, date, date, integer, integer, text)

-- DROP FUNCTION financial_report_balance_ksamount_negative(integer, integer, date, date, integer, integer, text);

CREATE OR REPLACE FUNCTION financial_report_balance_ksamount_negative(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
...
--			IF  fiscal_year>0 THEN
--				sql := sql || ' AND fiscalyear.id between < $2';
--               	        END IF;
--                        retain = true;
--                           IF $5 = 'posted' THEN
--				       sql := sql || ' AND account_move.state = $5';
...
--  LANGUAGE plpgsql VOLATILE
--  COST 100;
--ALTER FUNCTION financial_report_balance_prior_current(integer, integer, integer, integer, text)
--  OWNER TO postgres;
...
--  LANGUAGE plpgsql VOLATILE
--  COST 100;
--ALTER FUNCTION financial_report_balance_current(integer, integer, date, date, integer, integer, text)
--  OWNER TO postgres;

-- Function: trial_balance_amount(integer, integer, date, date, integer, integer, text, text, text)

-- DROP FUNCTION trial_balance_amount(integer, integer, date, date, integer, integer, text, text, text);

CREATE OR REPLACE FUNCTION trial_balance_amount(account_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text, credit_debit_balance text, analytic text)
  RETURNS numeric AS
$BODY$
 DECLARE
    check_id boolean;
    ...
    sql text :=   'from account_move_line aml,
		    ...
		    account_analytic_account aa
                  WHERE
                    aml.period_id = period.id AND
                    aml.move_id = account_move.id AND
		    account_move.analytic_account_id=aa.id and 
                    period.fiscalyear_id = fiscalyear.id and 
		    aa.name=$8';

    query text :=   'from account_move_line aml,
                    account_period period,
                    account_fiscalyear fiscalyear,
                    account_move
                  WHERE
  ...
  BEGIN
	...
	IF analytic IS NOT NULL THEN 
		   ...
		   IF end_period_id >0  and start_period_id is null and end_period_id is not null THEN
		    ...
		    sql = sql_cond|| sql;
		    RAISE NOTICE ' SQL (%)', sql;
		    EXECUTE sql into amount USING account_id,fiscal_year,start_date,end_date,start_period_id,end_period_id,state_cond,analytic;
		    IF amount < 0 THEN
		    amount = amount;
		    END IF;
	...
	ELSE
				...
				END IF;
				 ...
				 IF account_id IS NOT NULL THEN
					query := query || ' AND aml.account_id =$1';
				    END IF;
				    
		    IF  fiscal_year>0 THEN
		      query := query || ' AND fiscalyear.id = $2';
		    END IF;

		    IF  start_date > end_date and start_date is not null and end_date is not null THEN
		      query := query || ' AND account_move.date between $3 and $4';
		    END IF;
		    ...
		    IF  end_date is null and start_date is not null THEN
		      query := query || ' AND account_move.date = $3';
		    END IF;

		     IF   start_date is null and end_date is not null THEN
		      query := query || ' AND account_move.date = $4';
		    END IF;

		    IF start_period_id >0  and end_period_id >0 and start_period_id is not null and end_period_id is not null  THEN
		      query := query || ' AND aml.period_id between $5 and $6';
		    END IF;
		    
		   IF start_period_id >0  and end_period_id is null and start_period_id is not null THEN
		      query := query || ' AND aml.period_id = $5 ';
		    END IF;
		       
		   IF end_period_id >0  and start_period_id is null and end_period_id is not null THEN
		      query := query || ' AND aml.period_id = $6 ';
		    END IF;
		    
		    IF $7 = 'posted' THEN
		       query :=query || ' AND account_move.state = $7';
		    END IF;
		    query = sql_cond|| query;
		    RAISE NOTICE ' SQL (%)', query;
		    EXECUTE query into amount USING account_id,fiscal_year,start_date,end_date,start_period_id,end_period_id,state_cond,analytic;
		    IF amount < 0 THEN
		    amount = amount;
		    END IF;
...
-- Function: financial_report_monthly(integer, integer, integer, integer, text)

-- DROP FUNCTION financial_report_monthly(integer, integer, integer, integer, text);

CREATE OR REPLACE FUNCTION financial_report_monthly(report_id integer, fiscal_year integer, start_period_id integer, end_period_id integer, state_cond text)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
...
-- Function: financial_report_balance_monthly_prior_current(integer, integer, integer, integer, text)

-- DROP FUNCTION financial_report_balance_monthly_prior_current(integer, integer, integer, integer, text);

CREATE OR REPLACE FUNCTION financial_report_balance_monthly_prior_current(report_id integer, fiscal_year integer, start_period_id integer, end_period_id integer, state_cond text)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
  ...
  BEGIN
			...
			IF  fiscal_year>0 THEN
				...
				sql := sql || ' AND aml.account_id in ('|| array_to_string(current_period_account_ids, ',') ||')';
				IF start_period_id >0  and end_period_id >0 and start_period_id is not null and end_period_id is not null  THEN
					sql := sql ||  ' AND aml.period_id>= yyear-1 and aml.period_id <= $4';
				END IF;
                        END IF;
			IF $5 = 'posted' THEN
			       sql := sql || ' AND account_move.state = $5';
...
-- Function: financial_report_balance_analytic_bl(integer, integer, date, date, integer, integer, text)

-- DROP FUNCTION financial_report_balance_analytic_bl(integer, integer, date, date, integer, integer, text);

CREATE OR REPLACE FUNCTION financial_report_balance_analytic_bl(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
    ...
    r account_account_financial_report%rowtype;
    sql text :=   'SELECT 
			COALESCE(sum((aml.debit - aml.credit)),0.00)
		    from account_move_line aml,
                    account_period period,
                    account_fiscalyear fiscalyear,
                    account_move
...
-- Function: currency_account_report(character varying, character varying, date, date)

-- DROP FUNCTION currency_account_report(character varying, character varying, date, date);

CREATE OR REPLACE FUNCTION currency_account_report(IN param_state character varying, IN param_journal character varying, IN from_date date, IN to_date date)
  RETURNS TABLE(s_id integer, t_date date, t_id character varying, t_type character varying, dsc character varying, is_qty integer, r_qty integer, bln integer) AS
$BODY$
declare 
	stock RECORD ;
	product_data RECORD;
	...
	--Deleting Temp Table
		...
		DELETE FROM account_temp_report;
		select round(coalesce(sum(debit+credit),0)) into opening_balance from (
		select AM.date,AM.ref as ref,RP.name  as name,AML.name as description,0 as debit, AML.amount_currency as credit
		from account_move AM,account_move_line AML,account_journal AJ,res_partner RP
		where AM.id=AML.move_id
		and AM.journal_id=AJ.id
		and AJ.default_debit_account_id=AML.account_id
		and AML.amount_currency<0
		and AML.partner_id=RP.id
		and AM.state=param_state
		and AJ.name=param_journal
		and AM.date<from_date
		Union all
		select AM.date,AM.ref as ref,null  as name,AML.name as description,0 as debit, AML.amount_currency as credit
		from account_move AM,account_move_line AML,account_journal AJ
		where AM.id=AML.move_id
		and AM.journal_id=AJ.id
		and AJ.default_debit_account_id=AML.account_id
		and AML.amount_currency<0
		...
		Union all
		select AM.date,AM.ref as ref,RP.name as name,AML.name as description,AML.amount_currency as debit,0 as credit
		from account_move AM,account_move_line AML,account_journal AJ,res_partner RP
		where AM.id=AML.move_id
		and AM.journal_id=AJ.id
		and AJ.default_debit_account_id=AML.account_id
		...
		and AM.date<from_date
		Union all
		select AM.date,AM.ref as ref,null as name,AML.name as description,AML.amount_currency as debit,0 as credit
		from account_move AM,account_move_line AML,account_journal AJ
		where AM.id=AML.move_id
		and AM.journal_id=AJ.id
		and AJ.default_debit_account_id=AML.account_id
	...
	FOR trans_data 
		...
		IN
		select date,ref,name,date,description,debit,credit
		from
		(
			select AM.date,AM.ref as ref,RP.name  as name,AML.name as description,0 as debit, -(AML.amount_currency) as credit
			from account_move AM,account_move_line AML,account_journal AJ,res_partner RP
	...
	LOOP
		...
		if tran_ref = '-' then
			...
			update account_temp_report set trans_id = trans_data.ref,trans_type=trans_data.name,
			description = 	trans_data.description,rec_qty = trans_data.credit,
			iss_qty = trans_data.debit where trans_date = trans_data.date ;
			...
			INSERT INTO account_temp_report(trans_date,trans_id,trans_type,description, rec_qty, 
				    iss_qty,balance)
			    VALUES (trans_data.date,trans_data.ref,trans_data.name, trans_data.description, trans_data.credit, 
				    trans_data.debit,opening_balance);	
		end if;
	END LOOP;
	FOR row_data 
	IN
	...
	RETURN QUERY select  id as s_id,trans_date as t_date, trans_id as ref, trans_type as type, description as dsc,  
			    iss_qty as is_qty ,rec_qty as r_qty,balance as bln from account_temp_report order by t_date,s_id desc;
...
-- Function: currency_account_report_sdpt(integer, character varying, character varying, date, date)

-- DROP FUNCTION currency_account_report_sdpt(integer, character varying, character varying, date, date);

CREATE OR REPLACE FUNCTION currency_account_report_sdpt(IN param_fiscal_id integer, IN param_status character varying, IN param_account character varying, IN from_date date, IN to_date date)
  RETURNS TABLE(s_id integer, t_date date, t_id character varying, t_type character varying, dsc character varying, is_qty numeric, r_qty numeric, bln numeric, p_state character varying) AS
$BODY$
declare 
	stock RECORD ;
...
-- 		select COALESCE(sum(aml.amount_currency),0) into opening_balance
-- 		from account_move am,account_move_line aml,account_account aa,account_fiscalyear year,account_period period
-- 		where am.id=aml.move_id
-- 		and aa.id=aml.account_id
-- 		and period.id=aml.period_id
-- 		and period.fiscalyear_id=year.id
-- 		and aa.name=param_account
-- 		and year.id=param_fiscal_id
-- 		--and am.period_id!=(select id from account_period where fiscalyear_id=param_fiscal_id and name like '%Opening Period%')
-- 		and am.date<from_date;


		select COALESCE(sum(aml.amount_currency),0) into opening_balance
		from account_move am,account_move_line aml,account_account aa
		where am.id=aml.move_id
		and aa.id=aml.account_id
		and aa.name=param_account
		and am.period_id!=(select id from account_period where fiscalyear_id=param_fiscal_id and name like '%Opening Period%')
	...
	LOOP
		...
		select trans_id into tran_ref from account_temp where trans_date = trans_data.date order by trans_data.date;
		if tran_ref = '-' then
				
			update account_temp set trans_id = trans_data.ref,trans_type=trans_data.name,
			description = 	trans_data.description,rec_qty = trans_data.credit,
			iss_qty = trans_data.debit,state=trans_data.state where trans_date = trans_data.date ;
	 		   
		else
		
			INSERT INTO account_temp(trans_date,trans_id,trans_type,description, rec_qty, 
				    iss_qty,balance,state)
			    VALUES (trans_data.date,trans_data.ref,trans_data.name, trans_data.description, trans_data.credit, 
				    trans_data.debit,opening_balance,trans_data.state);	
		end if;
	...
	FOR row_data 
	IN
		select rec_qty,iss_qty,coalesce(balance,0) as balance,id from account_temp order by trans_date,id desc

	LOOP    
	        delete from account_temp where description='-'and rec_qty=0 and iss_qty=0;
		...
		if new_balance is null then
			select row_data.balance+(row_data.iss_qty-row_data.rec_qty) into new_balance from account_temp; 
			--where id = row_data.id;
			RAISE NOTICE 'new_balance=0.......%',new_balance;		
			update account_temp set balance = new_balance
			where id = row_data.id;		
			...
			RAISE NOTICE 'id-1.......%',row_data.id;
			RAISE NOTICE 'new_balance.......%',new_balance;		
			update account_temp set balance = new_balance where id = row_data.id;			
	...
	RETURN QUERY select  id as s_id,trans_date as t_date, trans_id as ref, trans_type as type, description as dsc,  
			    iss_qty as is_qty ,rec_qty as r_qty,balance as bln,state as p_state from 
			    account_temp where state = param_status order by t_date,s_id desc
			    ;
	...
	RETURN QUERY select  id as s_id,trans_date as t_date, trans_id as ref, trans_type as type, description as dsc,  
			    iss_qty as is_qty ,rec_qty as r_qty,balance as bln,state as p_state from 
			    account_temp  order by t_date,s_id desc 
			    ;
	end if;
	END;
$BODY$
...
-- Function: balance_sheet_report_last_year_multicompany(integer, integer, text, integer)

-- DROP FUNCTION balance_sheet_report_last_year_multicompany(integer, integer, text, integer);

CREATE OR REPLACE FUNCTION balance_sheet_report_last_year_multicompany(report_id integer, start_fiscal_year integer, state_cond text, company_id integer)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
  ...
  BEGIN
    ...
    IF $3 = 'posted' THEN
       sql := sql || ' AND account_move.state = $3';
    END IF;
    sql := sql || ' AND (account_move.company_id in (select res.id from res_company res where res.id=$4) or account_move.company_id in (select res.id from res_company res where res.parent_id=$4)) group by aml.account_id';
    RAISE NOTICE ' SQL (%)', sql;
    
    EXECUTE sql into balance USING account_ids,start_fiscal_year,state_cond,company_id;
    --IF balance < 0 THEN
    --balance = balance * -1;
    --END IF;
...
-- Function: balance_sheet_report_this_year_multicompany(integer, integer, text, integer)

-- DROP FUNCTION balance_sheet_report_this_year_multicompany(integer, integer, text, integer);

CREATE OR REPLACE FUNCTION balance_sheet_report_this_year_multicompany(report_id integer, start_fiscal_year integer, state_cond text, company_id integer)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
  ...
  BEGIN
                    ...
                    FOR r IN select *  from account_account_financial_report where report_line_id=$1
                    LOOP
			RAISE NOTICE 'r.account_id(%)',r.account_id;
			select array_cat(account_ids,array_agg(id)) into account_ids from account_tree where r.account_id = ANY(PATH);
                    END LOOP;
...
-- Function: financial_report_balance_analytic_current_multicompany(integer, integer, date, date, integer, integer, text, integer, integer)

-- DROP FUNCTION financial_report_balance_analytic_current_multicompany(integer, integer, date, date, integer, integer, text, integer, integer);

CREATE OR REPLACE FUNCTION financial_report_balance_analytic_current_multicompany(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text, analytic_id integer, company_id integer)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
  ...
  BEGIN  
   ...
   IF end_period_id >0  and start_period_id is null and end_period_id is not null THEN
    ...
    IF $7 = 'posted' THEN
       sql := sql || ' AND account_move.state = $7';
    END IF;
    sql := sql || ' AND (account_move.company_id in (select res.id from res_company res where res.id='|| company_id || ') or account_move.company_id in (select res.id from res_company res where res.parent_id='|| company_id || ')) group by aml.account_id';
    EXECUTE sql into balance USING analytic_account_ids,fiscal_year,start_date,end_date,start_period_id,end_period_id,state_cond;
    IF balance < 0 THEN
    balance = balance * -1;
    END IF;
...
-- Function: financial_report_balance_analytic_multicompany(integer, integer, date, date, integer, integer, text, integer, integer)

-- DROP FUNCTION financial_report_balance_analytic_multicompany(integer, integer, date, date, integer, integer, text, integer, integer);

CREATE OR REPLACE FUNCTION financial_report_balance_analytic_multicompany(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text, analytic_id integer, company_id integer)
  RETURNS numeric AS
$BODY$
 DECLARE
   balance numeric;
 ...
 BEGIN  
   ...
   sql := sql || ' AND (account_move.company_id in (select res.id from res_company res where res.id='|| company_id || ') or account_move.company_id in (select res.id from res_company res where res.parent_id='|| company_id || '))';
...
-- Function: financial_report_balance_analytic_prior_multicompany(integer, integer, integer, integer, text, integer, integer)

-- DROP FUNCTION financial_report_balance_analytic_prior_multicompany(integer, integer, integer, integer, text, integer, integer);

CREATE OR REPLACE FUNCTION financial_report_balance_analytic_prior_multicompany(report_id integer, fiscal_year integer, start_period_id integer, end_period_id integer, state_cond text, analytic_id integer, company_id integer)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
...
-- Function: financial_report_balance_current_multicompany(integer, integer, date, date, integer, integer, text, integer)

-- DROP FUNCTION financial_report_balance_current_multicompany(integer, integer, date, date, integer, integer, text, integer);

CREATE OR REPLACE FUNCTION financial_report_balance_current_multicompany(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text, company_id integer)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
  ...
  BEGIN
                      select  id into period from account_period where fiscalyear_id =$2 and special is false order by id limit 1;
                      start_period_id = period ;
                     -- select account_id from account_account_financial_report where report_line_id=report_id
                    FOR r IN select *  from account_account_financial_report where report_line_id=$1
                    LOOP
   ...
   IF end_period_id >0  and start_period_id is null and end_period_id is not null THEN
    ...
    END IF;
    RAISE NOTICE ' SQL (%)', sql;
    sql := sql || ' AND (account_move.company_id in (select res.id from res_company res where res.id=$8) or account_move.company_id in (select res.id from res_company res where res.parent_id=$8)) group by aml.account_id';
    EXECUTE sql into balance USING account_ids,fiscal_year,start_date,end_date,start_period_id,end_period_id,state_cond,company_id,company_id;
    IF balance < 0 THEN
    balance = balance*-1;
    END IF;
        RAISE NOTICE 'balance (%)', balance;
...
-- Function: financial_report_balance_mutlicompany(integer, integer, date, date, integer, integer, text, integer)

-- DROP FUNCTION financial_report_balance_mutlicompany(integer, integer, date, date, integer, integer, text, integer);

CREATE OR REPLACE FUNCTION financial_report_balance_mutlicompany(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text, company_id integer)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
    ...
    temp_account_id int[];
    r account_account_financial_report%rowtype;
    rr account_account_consol_rel%rowtype;
    sql text :=   'SELECT case when aml.account_id in (select aa.id from account_account aa , account_account_type  acc_type where aa.user_type = acc_type.id and acc_type.report_type in (''income'',''liability'')) then  COALESCE(sum((aml.credit - aml.debit)),0.00)
		    else COALESCE(sum((aml.debit - aml.credit)),0.00)
		    end as financial_report_balance
		    from account_move_line aml,
  ...
  BEGIN
                    ...
                    raise NOTICE ' searching id(%)',r.account_id;
			select array_cat(account_ids,array_agg(id)) into account_ids from account_tree where r.account_id = ANY(PATH);
			FOR rr in select * from account_account_consol_rel where child_id =r.account_id
			LOOP 
				select array_append(account_ids,rr.parent_id) into account_ids;
			END LOOP;				
                    END LOOP;
                    raise NOTICE ' account_ids(%)',account_ids;
                    --there is not account and return null for that
...
-- Function: financial_report_balance_prior_mutlicompany(integer, integer, integer, integer, text, integer)

-- DROP FUNCTION financial_report_balance_prior_mutlicompany(integer, integer, integer, integer, text, integer);

CREATE OR REPLACE FUNCTION financial_report_balance_prior_mutlicompany(report_id integer, fiscal_year integer, start_period_id integer, end_period_id integer, state_cond text, company_id integer)
  RETURNS numeric AS
$BODY$
  DECLARE
    balance numeric;
...
-- Function: financial_report_balance_analytic_sir(integer, integer, date, date, integer, integer, text, integer)

-- DROP FUNCTION financial_report_balance_analytic_sir(integer, integer, date, date, integer, integer, text, integer);

-- CREATE OR REPLACE FUNCTION financial_report_balance_analytic_sir(report_id integer, fiscal_year integer, start_date date, end_date date, start_period_id integer, end_period_id integer, state_cond text, analytic_id integer)
  -- RETURNS numeric AS
-- $BODY$
  -- DECLARE
    -- balance numeric;
  ...
  -- BEGIN  
			    ...
			    -- IF analytic_account_ids IS NOT NULL THEN
				-- sql := sql || ' AND aml.analytic_account_id in ('|| array_to_string(analytic_account_ids, ',') ||')';
				-- sql := sql || ' AND aml.analytic_account_id in (select account_analytic_id from account_financial_report_analytic_account_rel where account_financial_report_id in(select account_financial_report_id from account_financial_report_analytic_account_rel where account_analytic_id=' || analytic_id || '))';
			    -- END IF;
			    -- RAISE NOTICE 'sql(%)',sql;	
    -- IF  fiscal_year>0 THEN
      -- sql := sql || ' AND fiscalyear.id = $2';
...
$BODY$
  ...
  BEGIN  
    ...
    RAISE NOTICE 'analytic_id(%)',analytic_id;
       select array_agg(id) into analytic_account_ids 
       from analytic_report_tree  where analytic_id= ANY(PATH);
    ...
    --sql := sql || ' AND aml.analytic_account_id is not null';
    sql := sql || ' AND aml.analytic_account_id in (select account_analytic_id from account_financial_report_analytic_account_rel where account_financial_report_id=' || report_id || 'group by account_analytic_id)';
       END IF;
       IF analytic_account_ids IS NULL THEN
    RAISE NOTICE 'analytic_account_id is null(%)',analytic_account_ids;
    sql := sql || ' AND aml.analytic_account_id is not null';
       END IF;
    ...
    -- sql := sql || ' AND aml.analytic_account_id in ('|| array_to_string(analytic_account_ids, ',') ||')';
    --sql := sql || ' AND aml.analytic_account_id in (select account_analytic_id from account_financial_report_analytic_account_rel where account_financial_report_id in(select account_financial_report_id from account_financial_report_analytic_account_rel where account_analytic_id=' || analytic_id || '))';
    sql := sql || ' AND aml.analytic_account_id in (select account_analytic_id from account_financial_report_analytic_account_rel where account_analytic_id=' || analytic_id || 'group by account_analytic_id)';
    RAISE NOTICE 'analytic_account_id is not null(%)',analytic_id;
       END IF;
       RAISE NOTICE 'sql(%)',sql; 
    IF  fiscal_year>0 THEN
      sql := sql || ' AND fiscalyear.id = $2';
