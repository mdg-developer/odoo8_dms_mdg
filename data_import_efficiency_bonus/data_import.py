...
header_fields = ['fingerprint_id','efficiency_bonus','date_from','date_to']

class employee_efficiency_bonus(osv.osv):
    _name = 'data_import.efficiency_bonus'
    _columns = {
...
#                 else:
                if ln:
                    ...
                    fingerprint_id_i = efficiency_bonus_i = number_of_hours_i = number_of_days_i = date_from_i = date_to_i = None
                    column_cnt = len(ln)
                    ...
                    for i in range(column_cnt):
                        ...
                        elif header_field == 'efficiency_bonus':
                            efficiency_bonus_i = i
                ...
                if ln and ln not in ['#','']:
                    ...
                    import_vals['efficiency_bonus'] = ln[efficiency_bonus_i]                    
...
#                 corporate_title = function_title = contract_type = pay_ids = base_ids  =marital_status = line = skill = father = tax_father =tax_father_type =mother = tax_mother = tax_mother_type =spouse = tax_spouse =nric =highest_qual =None
                fingerprint_id = efficiency_bonus = date_from = date_to = None
                value ={}
                ...
                if aml['efficiency_bonus']:
                    efficiency_bonus = aml['efficiency_bonus']
                ...
                if payslip_id:
                    ...
                    print 'search hr_payslip_inputs_days'
                    cr.execute ("""select id from hr_payslip_input where payslip_id= %s and code='EBS'""",(payslip_id,))
                    workd_id = cr.fetchall()
                    
                if workd_id:  
                    cr.execute('update hr_payslip_input set amount=%s where id=%s',(efficiency_bonus,workd_id[0][0],))
