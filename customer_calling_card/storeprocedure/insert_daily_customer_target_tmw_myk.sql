-- select * from insert_daily_customer_target_tmw_myk();
-- DROP FUNCTION insert_daily_customer_target_tmw_myk();

CREATE OR REPLACE FUNCTION insert_daily_customer_target_tmw_myk()
  RETURNS void AS
$BODY$
...
BEGIN
	...
	for customer in select rp.id,rp.name,customer_code,outlet.id outlet,rc.id city,rt.id township,street,rb.id branch,
					...
					and rb.branch_code='TMW' and rt.code='MYK'
	loop			
