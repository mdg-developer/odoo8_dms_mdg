-- select * from insert_daily_customer_target_yip_hgu();
-- DROP FUNCTION insert_daily_customer_target_yip_hgu();

CREATE OR REPLACE FUNCTION insert_daily_customer_target_yip_hgu()
  RETURNS void AS
$BODY$
...
BEGIN
	...
	for customer in select rp.id,rp.name,customer_code,outlet.id outlet,rc.id city,rt.id township,street,rb.id branch,
					...
					and rb.branch_code='YIP' and rt.code='HGU'
	loop			
