--select * from get_customer_target(28477)
--select * from get_customer_target('46997,28477')
--select * from get_customer_target(46997)

CREATE OR REPLACE FUNCTION get_customer_target(IN param_customer integer)
...
$BODY$
	...
	sql text := 'with product_price as (
				...
				product_trans as (
					select COALESCE(sum(total_value),0.00) total_value,customer_id partner_id
					...
					(	select *,
						...
						(	select *,
							...
							(	select *,
								...
								(select ti.target_amount from target_info ti where ti.outlet_id=BB.outlet_type and ti.product_id=BB.product_id)
								from
								(	select partner_id,customer_code,outlet_type,city,township,street,branch_id,frequency_id,class_id,
									sales_channel,delivery_team_id,product_id,uom_id,unit_price,sum(month1_sale) month1_sale,sum(month2_sale) month2_sale,
									sum(month3_sale) month3_sale,sum(ach_qty) ach_qty,
									sum(month1_sale)+sum(month2_sale)+sum(month3_sale) total_sale_qty,
									case when sum(month1_sale) = 0 and sum(month2_sale) = 0 and sum(month3_sale) = 0 then 0
										 when sum(month1_sale) > 0 and sum(month2_sale) > 0 and sum(month3_sale) > 0 then 3 
										 when (sum(month1_sale) > 0 and sum(month2_sale) > 0) or (sum(month1_sale) > 0 and sum(month3_sale) > 0) or (sum(month2_sale) > 0 and sum(month3_sale) > 0) then 2 
										 else 1 	 
									end as divisor
									from
									(	select 
										...
										case when month1_rec.product_id is not null then (select COALESCE(new_price,0.00) from product_price where product_id=month1_rec.product_id and product_uom_id=month1_rec.uom_id) 
											 when month2_rec.product_id is not null then (select COALESCE(new_price,0.00) from product_price where product_id=month2_rec.product_id and product_uom_id=month2_rec.uom_id) 
											 when month3_rec.product_id is not null then (select COALESCE(new_price,0.00) from product_price where product_id=month3_rec.product_id and product_uom_id=month3_rec.uom_id) 
											 when current_rec.product_id is not null then (select COALESCE(new_price,0.00) from product_price where product_id=current_rec.product_id and product_uom_id=current_rec.uom_id) 
										end as unit_price,
										COALESCE(month1_sale,0.00) month1_sale,COALESCE(month2_sale,0.00) month2_sale,
										COALESCE(month3_sale,0.00) month3_sale,COALESCE(CEIL(product_quantity),0.00) ach_qty
										from
										(
											select COALESCE(sum(product_quantity),0.00) month1_sale,partner_id,customer_code,outlet_type,township,city,street,branch_id,
											...
											(   select 
												...
												and foc!=true and inv.partner_id = ''' || param_customer || '''
										...
										full outer join
										(
											select COALESCE(sum(product_quantity),0.00) month2_sale,partner_id,customer_code,outlet_type,city,
										...
										full outer join
										(
											select COALESCE(sum(product_quantity),0.00) month3_sale,partner_id,customer_code,outlet_type,city,
										...
										full outer join
										(
											select COALESCE(sum(product_quantity),0.00) product_quantity,partner_id,customer_code,outlet_type,
									...
									sales_channel,delivery_team_id,product_id,uom_id,unit_price
								)BB
							)CC
						)DD
					)EE
