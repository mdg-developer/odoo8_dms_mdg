-- select * from insert_daily_customer_target_yip_other();
-- DROP FUNCTION insert_daily_customer_target_yip_other();

CREATE OR REPLACE FUNCTION insert_daily_customer_target_yip_other()
  RETURNS void AS
$BODY$
...
BEGIN
	...
	for customer in select rp.id,rp.name,customer_code,outlet.id outlet,rc.id city,rt.id township,street,rb.id branch,
					...
					and rb.branch_code='YIP' and rt.code in ('INS','TMW','AHL','HLG','BTT','YKN','BHN','TGG','TNL','SDG','TKT','MYG','PZT','DGN','SCG','DSK','DBN','SOK','KMD','HTY','BGO','TGO')
	loop			
		...
		raise notice 'inserted customer target';
		...
		loop
			...
			avg_sale = 0;
			product_price = 0;
			percentage_growth_value = 0;
			target_amount = 0;
			percentage_growth_amount = 0;
			final_ams = 0;
			customer_ams = 0;
			current_month_sale = 0;
			ach_percent = 0;
						
			select COALESCE(sum(product_quantity),0) into month1_sale
			from 
			(   select 
			...
			select COALESCE(sum(product_quantity),0) into month2_sale
			from 
			(   select 
				(quantity*(select floor(round(1/factor,2)) as ratio from product_uom where active = true and id=inv_line.uos_id)) as product_quantity
				from account_invoice_line inv_line,account_invoice inv,product_product pp,product_template pt
			...
			select COALESCE(sum(product_quantity),0) into month3_sale
			from 
			(   select 
				(quantity*(select floor(round(1/factor,2)) as ratio from product_uom where active = true and id=inv_line.uos_id)) as product_quantity
				from account_invoice_line inv_line,account_invoice inv,product_product pp,product_template pt
			...
			select new_price into product_price
			...
			select COALESCE(sum(product_quantity),0) product_quantity into current_month_sale
			from 
			(   select 
				quantity*(select floor(round(1/factor,2)) as ratio from product_uom where active = true and id=inv_line.uos_id) as product_quantity
				from account_invoice_line inv_line,account_invoice inv,product_product pp,product_template pt
			...
			select COALESCE(sum(total_value),0) into month_out_todate_data
			...
			avg_sale*product_price,customer_ams,current_month_sale,ach_percent,customer_ams - current_month_sale);
			
			update customer_target
			set ams_total=ams_total_data,
				ams_buget_total=ams_budget_value_data,
				month_out_todate=month_out_todate_data,
				ams_balance=ams_budget_value_data-month_out_todate_data
			where id=target_id;
