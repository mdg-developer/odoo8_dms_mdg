-- select * from insert_daily_customer_target_yip_dla();
-- DROP FUNCTION insert_daily_customer_target_yip_dla();

CREATE OR REPLACE FUNCTION insert_daily_customer_target_yip_dla()
  RETURNS void AS
$BODY$
...
BEGIN
	...
	for customer in select rp.id,rp.name,customer_code,outlet.id outlet,rc.id city,rt.id township,street,rb.id branch,
					...
					and rb.branch_code='YIP' and rt.code='DLA'
	loop			
