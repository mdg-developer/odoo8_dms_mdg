-- select * from insert_daily_customer_target_mdy_ptg();
-- DROP FUNCTION insert_daily_customer_target_mdy_ptg();

CREATE OR REPLACE FUNCTION insert_daily_customer_target_mdy_ptg()
  RETURNS void AS
$BODY$
...
BEGIN
	...
	for customer in select rp.id,rp.name,customer_code,outlet.id outlet,rc.id city,rt.id township,street,rb.id branch,
					...
					and rb.branch_code='MDY' and rt.code='PTG'
	loop			
