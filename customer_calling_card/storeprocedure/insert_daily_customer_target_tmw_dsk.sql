-- select * from insert_daily_customer_target_tmw_dsk();
-- DROP FUNCTION insert_daily_customer_target_tmw_dsk();

CREATE OR REPLACE FUNCTION insert_daily_customer_target_tmw_dsk()
  RETURNS void AS
$BODY$
...
BEGIN
	...
	for customer in select rp.id,rp.name,customer_code,outlet.id outlet,rc.id city,rt.id township,street,rb.id branch,
					...
					and rb.branch_code='TMW' and rt.code='DSK'
	loop			
