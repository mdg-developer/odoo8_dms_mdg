-- select * from insert_daily_customer_target_mdy_sng();
-- DROP FUNCTION insert_daily_customer_target_mdy_sng();

CREATE OR REPLACE FUNCTION insert_daily_customer_target_mdy_sng()
  RETURNS void AS
$BODY$
...
BEGIN
	...
	for customer in select rp.id,rp.name,customer_code,outlet.id outlet,rc.id city,rt.id township,street,rb.id branch,
					...
					and rb.branch_code='MDY' and rt.code='SNG'
	loop			
