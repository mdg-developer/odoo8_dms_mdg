-- select * from insert_daily_customer_target_yip_lmd();
-- DROP FUNCTION insert_daily_customer_target_yip_lmd();

CREATE OR REPLACE FUNCTION insert_daily_customer_target_yip_lmd()
  RETURNS void AS
$BODY$
...
BEGIN
	...
	for customer in select rp.id,rp.name,customer_code,outlet.id outlet,rc.id city,rt.id township,street,rb.id branch,
					...
					and rb.branch_code='YIP' and rt.code='LMD'
	loop			
