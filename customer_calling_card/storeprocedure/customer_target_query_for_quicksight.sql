with 
					...
					(	select *,
						...
						(	select *,
							...
							(	select *,
								...
								--case when divisor > 0 then round(round((total_sale_qty/divisor),2)*unit_price,2) else round(total_sale_qty*unit_price,2)
								...
								(	select partner_id,customer_code,outlet_type,city,township,street,branch_id,frequency_id,class_id,
									...
									(	select 
										...
										--case when month1_rec.product_id is not null then (select COALESCE(new_price,0) from product_price where product_id=month1_rec.product_id and product_uom_id=month1_rec.uom_id) 
											-- when month2_rec.product_id is not null then (select COALESCE(new_price,0) from product_price where product_id=month2_rec.product_id and product_uom_id=month2_rec.uom_id) 
											-- when month3_rec.product_id is not null then (select COALESCE(new_price,0) from product_price where product_id=month3_rec.product_id and product_uom_id=month3_rec.uom_id) 
											 --when current_rec.product_id is not null then (select COALESCE(new_price,0) from product_price where product_id=current_rec.product_id and product_uom_id=current_rec.uom_id) 
										--end as unit_price,
				...
				select ROW_NUMBER () OVER (ORDER BY partner_id) id,partner_id,(select name from res_partner where id =partner_id ) as customer_name,(select name_template from product_product where id =product_id ) as product_name , (select name from product_uom where id =uom_id ) as product_uom,
				product_id,uom_id,ach_qty,COALESCE(target_qty,0) as target_qty,COALESCE(gap_qty,0) as gap_qty,
