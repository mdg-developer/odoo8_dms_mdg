-- select * from insert_daily_customer_target_customer(28477);
-- DROP FUNCTION insert_daily_customer_target_customer(integer)

CREATE OR REPLACE FUNCTION insert_daily_customer_target_customer(param_customer integer)
  RETURNS void AS
$BODY$
...
BEGIN
	...
	IF param_customer IS NOT NULL THEN
	
		delete from customer_target where partner_id=param_customer;
		
		sql := sql || ' AND rp.id ='|| param_customer;
