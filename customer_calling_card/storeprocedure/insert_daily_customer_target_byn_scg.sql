-- select * from insert_daily_customer_target_byn_scg();
-- DROP FUNCTION insert_daily_customer_target_byn_scg();

CREATE OR REPLACE FUNCTION insert_daily_customer_target_byn_scg()
  RETURNS void AS
$BODY$
...
BEGIN
	...
	for customer in select rp.id,rp.name,customer_code,outlet.id outlet,rc.id city,rt.id township,street,rb.id branch,
					...
					and rb.branch_code='BYN' and rt.code='SCG'
	loop			
