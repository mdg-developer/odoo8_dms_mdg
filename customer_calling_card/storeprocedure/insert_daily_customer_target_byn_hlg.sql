-- select * from insert_daily_customer_target_byn_hlg();
-- DROP FUNCTION insert_daily_customer_target_byn_hlg();

CREATE OR REPLACE FUNCTION insert_daily_customer_target_byn_hlg()
  RETURNS void AS
$BODY$
...
BEGIN
	...
	for customer in select rp.id,rp.name,customer_code,outlet.id outlet,rc.id city,rt.id township,street,rb.id branch,
					...
					and rb.branch_code='BYN' and rt.code='HLG'
	loop			
