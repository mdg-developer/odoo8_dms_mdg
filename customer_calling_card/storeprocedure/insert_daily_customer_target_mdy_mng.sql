-- select * from insert_daily_customer_target_mdy_mng();
-- DROP FUNCTION insert_daily_customer_target_mdy_mng();

CREATE OR REPLACE FUNCTION insert_daily_customer_target_mdy_mng()
  RETURNS void AS
$BODY$
...
BEGIN
	...
	for customer in select rp.id,rp.name,customer_code,outlet.id outlet,rc.id city,rt.id township,street,rb.id branch,
					...
					and rb.branch_code='MDY' and rt.code='MNG'
	loop			
