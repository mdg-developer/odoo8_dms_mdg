...
#                                          _('Initial Employment Date must be the same with Trial Start Date and Duration Start Date.\nEmployee: %s')% ee.name)
    ...
    def _get_working_service(self, cr, uid, ids, field_name, arg, context=None):

        res = dict.fromkeys(ids, 0.0)
        _res = self.work_service(cr, uid, ids, context=context)     
        for k, v in _res.iteritems():
            res[k] = v 
            if res[k]==(0.0, False):
                res[k]=""   
    ...
    _columns = {
        'initial_employment_date': fields.date('Initial Date of Employment', groups=False,required=True,
                                               help='Date of first employment if it was before the start of the first contract in the system.'),
        'length_of_service': fields.function(_get_employed_months, type='float', method=True, store=True,
                                             groups=False,
                                             string='Length of Service'),
        'working_service': fields.function(_get_working_service, type='char', method=True,
                                             groups=False,
                                             string='Working Service'),
    }
